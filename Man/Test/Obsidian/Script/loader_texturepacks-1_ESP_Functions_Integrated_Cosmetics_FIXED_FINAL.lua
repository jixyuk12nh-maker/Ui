    ...
    local function wrappedCreate(self, vmref)
        ...
        if owner == LocalPlayer and itemName ~= nil then
            ...
            if selection == nil then
                selection = {
                    skin = "NONE_COSMETIC",
                    wrap = { name = "NONE_COSMETIC", inverted = false },
                    charm = "NONE_COSMETIC",
                    finisher = "NONE_COSMETIC",
                }
            end
            ...
            if enumLib ~= nil then
                ...
                local sourceData = vmref[dataKey] or vmref.Data or {}
                local slot = {}
                for key, value in pairs(sourceData) do
                    slot[key] = value
                end
                local skinKey = enumLib:ToEnum("Skin")
                local wrapKey = enumLib:ToEnum("Wrap")
                local charmKey = enumLib:ToEnum("Charm")

                if selection.skin == "NONE_COSMETIC" then
                    slot[skinKey] = nil
                    slot.Skin = nil
                else
                    slot[skinKey] = { Name = selection.skin }
                    slot.Skin = nil
                end

                if selection.wrap and selection.wrap.name == "NONE_COSMETIC" then
                    slot[wrapKey] = nil
                    slot.Wrap = nil
                elseif selection.wrap and selection.wrap.name then
                    slot[wrapKey] = { Name = selection.wrap.name, Inverted = selection.wrap.inverted == true }
                    slot.Wrap = nil
                end

                if selection.charm == "NONE_COSMETIC" then
                    slot[charmKey] = nil
                    slot.Charm = nil
                elseif selection.charm then
                    slot[charmKey] = { Name = selection.charm }
                    slot.Charm = nil
                end
...
local function initDefaultSelections()
    local itemLib = CosmeticsWrap.itemLibrary()
    if itemLib == nil or typeof(itemLib.Items) ~= "table" then
        warn("[CosmeticsWrap] ItemLibrary unavailable for default selections")
        return
    end

    for weaponName, data in pairs(itemLib.Items) do
        if typeof(data) == "table" and data.Class ~= "Utility" then
            if CosmeticsWrap.selections[weaponName] == nil then
                CosmeticsWrap.selections[weaponName] = {
                    skin = "NONE_COSMETIC",
                    wrap = { name = "NONE_COSMETIC", inverted = false },
                    charm = "NONE_COSMETIC",
                    finisher = "NONE_COSMETIC",

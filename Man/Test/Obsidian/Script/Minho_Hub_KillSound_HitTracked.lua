...
local function installHitSound()
    ...
    viewmodelModule.PlayHitmarkerSound = function(self, crit, divisor, ...)
        ...
        -- ★ 히트 시점에 대상 Humanoid 기록 (킬사운드용)
        -- killSoundState는 아래에서 선언되므로 getgenv()로 직접 참조한다.
        ...
        if container then
            for _, model in ipairs(container:GetChildren()) do
                if model:IsA("Model") and model.Name == "Dummy" then
                    ...
                    local prev = killSoundState.LastHealth[model]
                    if prev and prev > 0 and hum.Health <= 0 then
                        local lastHit = killSoundState.RecentHits[hum]
                        if lastHit and (tick() - lastHit) <= killSoundState.HitWindow then
                            playKillSound()
                    ...
                    killSoundState.LastHealth[model] = hum.Health
                end
            end
        end

        -- 3) 오래된 히트 기록 정리
        for hum, t in pairs(killSoundState.RecentHits) do
            if tick() - t > killSoundState.HitWindow or not hum.Parent then
                killSoundState.RecentHits[hum] = nil
            end
        end
    end)

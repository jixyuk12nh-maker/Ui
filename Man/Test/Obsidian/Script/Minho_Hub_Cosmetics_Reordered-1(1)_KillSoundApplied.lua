...
local killSoundState = getgenv().__MinhoKillSoundState or {
    LastHealth = {},        -- [Player] = 마지막 체력
    Enabled = false,
    Conn = nil,             -- Heartbeat 연결
    PlayerConns = {},       -- [Player] = CharacterAdded 연결
    PlayerAdded = nil,
    _muteConn = nil,
}
...
local function installKillSoundSystem()
    ...
    for _, obj in ipairs(game:GetDescendants()) do
        task.spawn(muteOriginal, obj)
    end

    -- 새로 생기는 원본 킬사운드도 음소거
    local muteConn = game.DescendantAdded:Connect(function(obj)
        task.defer(muteOriginal, obj)
    end)
    ...
    -- uninstall용
    killSoundState._muteConn = muteConn
end
...
local function uninstallKillSoundSystem()
    ...
    if killSoundState._muteConn then
        killSoundState._muteConn:Disconnect()
        killSoundState._muteConn = nil
    end

...
local function getSpoofController()
    if SpoofBackend.Controller then
        return SpoofBackend.Controller
    end
...
local function setLocalAttribute(name, value)
    ...
    pcall(function()
        if SpoofState.AttributeOriginals[name] == nil then
            SpoofState.AttributeOriginals[name] = LocalPlayer:GetAttribute(name)
        end
...
local function restoreLocalAttributes()
    for attr, original in pairs(SpoofState.AttributeOriginals) do
        pcall(function()
            LocalPlayer:SetAttribute(attr, original)
        end)
    end
    table.clear(SpoofState.AttributeOriginals)
    table.clear(SpoofState.AttributeSpoofs)
end
...
local function applySpoofAttributes()
    ...
    for key, info in pairs(SPOOF_FLAGS) do
        if SpoofState.Flags[key] then
            ...
            local original = SpoofState.AttributeOriginals[info.Attribute]
            if original ~= nil then
                pcall(function() LocalPlayer:SetAttribute(info.Attribute, original) end)
            end
            SpoofState.AttributeSpoofs[info.Attribute] = nil
        end
    end
end
    ...
    local ok = pcall(function()
        oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, key)
            if self == LocalPlayer and SpoofState.Enabled then
                ...
                elseif key == "UserId" and tonumber(SpoofState.Values.UserId) then
                    return tonumber(SpoofState.Values.UserId)
                end
            end
...
local function installProfileProxy()
    ...
    local mt = {
        __index = function(_, key)
            if SpoofState.Enabled then
                ...
                if field and SpoofState.Values[field] ~= nil then
                    local value = SpoofState.Values[field]
                    if field == "CasualWinPercent" or field == "RankedWinPercent" then
                        return tonumber(value) and tonumber(value) / 100 or value
                    end
                    return value
                end
            end
    ...
    SpoofBackend.ProfileProxyInstalled = ok
end
...
-- Keep attributes alive while the UI is enabled. This does not create another UI.
task.spawn(function()
...
addSpoofInput(SpooferBox, "SpoofStatus", "Nametag Status", "NametagStatus", SpoofState.Values.NametagStatus)
addSpoofInput(SpooferStatsBox, "SpoofLevel", "Level", "Level", SpoofState.Values.Level)
...
addSpoofInput(SpooferStatsBox, "SpoofCasualWinPercent", "Casual Win %", "CasualWinPercent", SpoofState.Values.CasualWinPercent)
addSpoofInput(SpooferStatsBox, "SpoofRankedWinPercent", "Ranked Win %", "RankedWinPercent", SpoofState.Values.RankedWinPercent)

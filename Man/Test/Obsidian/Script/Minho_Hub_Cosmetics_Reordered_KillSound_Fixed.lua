...
-- Shared preview slot: both 3D previews live inside one layout item.
-- This prevents the hidden preview from reserving another 180px of vertical space.
...
task.spawn(function()
while task.wait() do
if CharmViewport.BackgroundTransparency < 1 and charmModel and charmAutoRotate then
...
task.spawn(function()
while task.wait() do
if WrapViewport.BackgroundTransparency < 1 and wrapModel and wrapAutoRotate then
...
wrapSpin = 0
wrapAutoRotate = true
WrapViewport.BackgroundTransparency = 0
else
...
CharmViewport.BackgroundTransparency = 0
else

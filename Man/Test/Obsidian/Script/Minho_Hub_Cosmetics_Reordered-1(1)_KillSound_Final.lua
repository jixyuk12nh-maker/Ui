...
local function installKillSoundSystem()
    ...
    -- 사운드 교체 함수
    local function applyKillSound(obj)
        if not obj:IsA("Sound") then return end
        local id = string.match(obj.SoundId, "%d+")
        if id == ORIGINAL_KILL_SOUND_ID then
            local selected = KILL_SOUNDS[soundState.KillSelected]
            if selected and selected ~= "" then
                pcall(function()
                    obj.SoundId = selected
                    obj.Volume = soundState.KillVolume
                    obj.PlaybackSpeed = soundState.KillPitch
                end)
            else
    ...
    for _, obj in ipairs(game:GetDescendants()) do
        task.spawn(applyKillSound, obj)
    end

    -- 새로 생기는 사운드도 감시
    killSoundState._muteConn = game.DescendantAdded:Connect(function(obj)
        task.defer(applyKillSound, obj)
    end)
end

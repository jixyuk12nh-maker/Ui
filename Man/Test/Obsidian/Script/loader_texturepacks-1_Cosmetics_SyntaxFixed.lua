...
local CosmeticsGroup = Cosmetics:AddLeftGroupbox("Rivals Cosmetics")
...
function CosmeticsWrap._require(key, fn)
    ...
    CosmeticsWrap._cache[key] = false
    return nil
end
...
function CosmeticsWrap.cosmeticLibrary()
    return CosmeticsWrap._require("CosmeticLibrary", function()
        local modules = ReplicatedStorage:FindFirstChild("Modules")
        local module = modules and modules:FindFirstChild("CosmeticLibrary")
        return module and require(module)
    end)
end
...
function CosmeticsWrap.assetsFolder(sub)
    ...
    return sub and assets:FindFirstChild(sub) or assets
end

function CosmeticsWrap.charmAssets()
    return CosmeticsWrap._require("CharmAssets", function()
        local assets = CosmeticsWrap.assetsFolder("Charms")
        return assets
    end)
end
...
function CosmeticsWrap.wrapPreviewAsset()
    return CosmeticsWrap._require("WrapPreviewAsset", function()
        ...
        return misc and misc:FindFirstChild("Wrap") or nil
    end)
end
...
if #classNames == 0 then
    CosmeticsCosmeticsGroup:AddLabel("코스메틱 데이터를 찾지 못했습니다.")
else
...
ViewerImage = CosmeticsCosmeticsGroup:AddImage("ViewerImage", {
Image = "rbxassetid://0",
Height = 220,
...
CharmViewport.Parent = CosmeticsCosmeticsGroup.Container
...
WrapViewport.Parent = CosmeticsCosmeticsGroup.Container
...
InfoLabel = CosmeticsCosmeticsGroup:AddLabel({ Text = "" })
...
-- None 이거나 nil이면 해당 무기 기본 이미지 표시 (스킨 미적용 상태)
...
--// 무기/종류/코스메틱 드롭다운 (검색 가능) \--
ClassDropdown = CosmeticsGroup:AddDropdown("ClassSelect", {
...
CosmeticDropdown:SetValue(NONE_LABEL) -- 기본값 None
end
...
WeaponDropdown = CosmeticsGroup:AddDropdown("WeaponSelect", {
Text = "Weapon",
...
KindDropdown = CosmeticsGroup:AddDropdown("KindSelect", {
Text = "Cosmetics",
...
CosmeticsGroup:AddButton("적용", function()
local weapon = WeaponDropdown.Value
local kind = KindDropdown.Value
...
CosmeticsGroup:AddButton("전체 초기화", function()

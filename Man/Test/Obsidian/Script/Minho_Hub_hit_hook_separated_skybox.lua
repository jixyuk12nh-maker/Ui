...
--// Spoofer backend + UI state
--// The UI only changes this state; the spoof logic below applies the values.
local SpoofState = getgenv().__MinhoSpoofState or {
    ...
    Values = {
        ...
        RankedElo = 3600,
        LeaderboardRank = 1,
        CasualWinPercent = 100,
        RankedWinPercent = 100,
        FavoriteMap = "Arena",
        NametagStatus = "Prime",
        Name = "Nosniy",
        DisplayName = "Nosniy",
    ...
    FieldEnabled = {
        ...
        CasualWinPercent = true,
        RankedWinPercent = true,
        FavoriteMap = true,
    },
    AttributeOriginals = {},
    AttributeOriginalExists = {},
    AttributeSpoofs = {},
}
getgenv().__MinhoSpoofState = SpoofState
for key, enabled in pairs({
    Name = true, DisplayName = true, UserId = true, NametagStatus = true,
    Level = true, Winstreak = true, CasualWins = true, RankedWins = true,
    RankedElo = true, LeaderboardRank = true, CasualWinPercent = true,
    RankedWinPercent = true, FavoriteMap = true,
}) do
    if SpoofState.FieldEnabled[key] == nil then
        SpoofState.FieldEnabled[key] = enabled
    end
end

-- Clean up a previous instance before installing a new backend/hook chain.
local PreviousSpoofBackend = getgenv().__MinhoSpoofBackend
if PreviousSpoofBackend then
    if PreviousSpoofBackend.IndexHook and type(restorefunction) == "function" then
        pcall(restorefunction, PreviousSpoofBackend.IndexHook)
    end
    local previousController = PreviousSpoofBackend.Controller
    local previousOriginalData = PreviousSpoofBackend.OriginalData
    if previousController and previousOriginalData and previousController.CurrentData then
        pcall(function()
            previousController.CurrentData.Data = previousOriginalData
        end)
    end
end
...
local SpoofBackend = {
    ...
    ProxyRef = nil,
    OriginalData = nil,
    LastControllerData = nil,
}
getgenv().__MinhoSpoofBackend = SpoofBackend

local SPOOF_ATTRIBUTES = {
    Level = "Level",
...
local SPOOF_FLAGS = {
    Influencer = { Attribute = "IsInfluencer", Value = true },
    RobloxEmployee = { Attribute = "IsRobloxEmployee", Value = true },
    NosniyTeam = { Attribute = "GroupRank", Value = 255 },
}
...
local PROFILE_FIELD_LOOKUP = {}
for spoofKey, profileKey in pairs(PROFILE_FIELDS) do
    PROFILE_FIELD_LOOKUP[profileKey] = spoofKey
end

local function getSpoofController()
    local cached = SpoofBackend.Controller
    if cached then
        local ok, current = pcall(function() return cached.CurrentData end)
        if ok and current then
            return cached
        end
        SpoofBackend.Controller = nil
    end
    ...
    if ok and ctrl then
        SpoofBackend.Controller = ctrl
        return ctrl
    end
    return nil
end

local function setLocalAttribute(name, value)
    if SpoofState.AttributeOriginalExists[name] == nil then
        local original = LocalPlayer:GetAttribute(name)
        SpoofState.AttributeOriginals[name] = original
        SpoofState.AttributeOriginalExists[name] = true
    end

    if SpoofState.AttributeSpoofs[name] == value then
        return
    end

    pcall(function()
        LocalPlayer:SetAttribute(name, value)
        SpoofState.AttributeSpoofs[name] = value
    end)
end

local function restoreAttribute(name)
    if not SpoofState.AttributeOriginalExists[name] then
        return
    end

    local original = SpoofState.AttributeOriginals[name]
    pcall(function()
        if original == nil then
            LocalPlayer:SetAttribute(name, nil)
        else
            LocalPlayer:SetAttribute(name, original)
        end
    end)
    SpoofState.AttributeSpoofs[name] = nil
end

local function restoreLocalAttributes()
    for attr in pairs(SpoofState.AttributeOriginalExists) do
        restoreAttribute(attr)
    end
    -- Keep the original snapshot so toggling the feature does not mistake a spoofed
    -- value for the real value. It is intentionally cleared only when this script
    -- instance is replaced/rejoined.
end

local function applySpoofAttributes()
    if not SpoofState.Enabled then
        restoreLocalAttributes()
        return
    end

    for key, attr in pairs(SPOOF_ATTRIBUTES) do
        if SpoofState.FieldEnabled[key] then
            setLocalAttribute(attr, SpoofState.Values[key])
        else
            restoreAttribute(attr)
        end
    end

    for key, info in pairs(SPOOF_FLAGS) do
        if SpoofState.Flags[key] then
            setLocalAttribute(info.Attribute, info.Value)
        else
            restoreAttribute(info.Attribute)
        end
    end
end

local function installPlayerInfoHook()
    if SpoofBackend.PlayerHookInstalled or type(hookmetamethod) ~= "function" then
        return
    end
    ...
    local ok = pcall(function()
        oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, key)
            if self == LocalPlayer and SpoofState.Enabled then
                if key == "Name" and SpoofState.FieldEnabled.Name and SpoofState.Values.Name ~= "" then
                    return SpoofState.Values.Name
                elseif key == "DisplayName" and SpoofState.FieldEnabled.DisplayName and SpoofState.Values.DisplayName ~= "" then
                    return SpoofState.Values.DisplayName
                elseif key == "UserId" and SpoofState.FieldEnabled.UserId then
                    local uid = tonumber(SpoofState.Values.UserId)
                    if uid then
                        return uid
                    end
                end
    ...
    if ok and oldIndex then
        SpoofBackend.IndexHook = oldIndex
        SpoofBackend.PlayerHookInstalled = true
    end
end

local function installProfileProxy()
    local ctrl = getSpoofController()
    if not ctrl or type(ctrl.CurrentData) ~= "table" or type(ctrl.CurrentData.Data) ~= "table" then
        return
    end

    local current = ctrl.CurrentData.Data
    -- Data is replaced by the game when player data is reloaded. Reinstall only when
    -- the underlying table has actually changed.
    if SpoofBackend.ProxyRef == current then
        return
    end

    local oldData = current
    local proxy = {}

    local mt = {
        __index = function(_, key)
            if SpoofState.Enabled then
                local field = PROFILE_FIELD_LOOKUP[key]
                local value = field and SpoofState.Values[field]
                if field and SpoofState.FieldEnabled[field] and value ~= nil then
                    if field == "CasualWinPercent" or field == "RankedWinPercent" then
                        local n = tonumber(value)
                        return n and (n / 100) or value
                    end
                    return value
                end
            end
            return rawget(oldData, key)
        end,
        __newindex = function(_, key, value)
            rawset(oldData, key, value)
        end,
        __iter = function()
            return next, oldData, nil
        end,
    }

    setmetatable(proxy, mt)
    local ok = pcall(function()
        ctrl.CurrentData.Data = proxy
    end)

    if ok then
        SpoofBackend.ProxyRef = proxy
        SpoofBackend.OriginalData = oldData
        SpoofBackend.LastControllerData = ctrl.CurrentData
    end
end

local function refreshSpoofer()
    installPlayerInfoHook()
    installProfileProxy()
    applySpoofAttributes()
end

-- Re-check periodically because the game's CurrentData/Data object can be replaced.
task.spawn(function()
    while LocalPlayer and LocalPlayer.Parent do
        if SpoofState.Enabled then
            pcall(refreshSpoofer)
        end
        task.wait(0.25)
    end
end)
...
local HIT_SOUNDS = {
    ...
    ["Headshot - CSGO"]="rbxassetid://5764885315",
}
...
local function applyFPSLighting()
    ...
    sky.SkyboxUp = fps.SkyboxUp
    sky.SkyboxFt = fps.SkyboxFt
    sky.SkyboxLf = fps.SkyboxLf
    sky.SkyboxDn = fps.SkyboxDn
    sky.SkyboxBk = fps.SkyboxBk
    sky.SkyboxRt = fps.SkyboxRt
    sky.SunAngularSize = fps.SunAngularSize
    sky.MoonAngularSize = fps.MoonAngularSize
...
HitSoundBox:AddDropdown("HitSoundSelect", {
    Text = "Hit Sound Type",
    Values = { "None","Neverlose","Gamesense","Skeet","Rust","Bell","Bubble","Minecraft","Osu","TF2", "Click Hit Sound", "Hit", "Headshot - CSGO" },
    Default = "None", Multi = false,
    Callback = function(Value) soundState.HitSelected = Value end
})

HitSoundBox:AddSlider("HitSoundVolume", {
    Text = "Hit Volume", Default = 0.5, Min = 0, Max = 2, Rounding = 1, Suffix = "x",
...
KillSoundBox:AddSlider("KillSoundVolume", {
    Text = "Kill Volume", Default = 0.5, Min = 0, Max = 2, Rounding = 1, Suffix = "x",
...
local SpooferBox = Spoofer:AddGroupbox({ Name = "Spoofer", Side = 1 })
local SpooferStatsBox = Spoofer:AddGroupbox({ Name = "Stats", Side = 2 })
local SpooferFlagsBox = Spoofer:AddGroupbox({ Name = "Flags", Side = 2 })

SpooferBox:AddCheckbox("SpooferEnabled", {
    Text = "Enabled",
    Default = SpoofState.Enabled,
    Callback = function(Value)
        SpoofState.Enabled = Value
        refreshSpoofer()
    end
})

local function addSpoofInput(box, id, text, key, default)
    box:AddCheckbox(id .. "Enabled", {
        Text = text,
        Default = SpoofState.FieldEnabled[key],
        Callback = function(Value)
            SpoofState.FieldEnabled[key] = Value
            if SpoofState.Enabled then refreshSpoofer() end
        end
    })

    box:AddInput(id, {
        Text = text .. " Value",
        Default = tostring(default ~= nil and default or ""),
        Callback = function(Value)
            SpoofState.Values[key] = Value
            if key ~= "Name" and key ~= "DisplayName" and key ~= "FavoriteMap" and key ~= "NametagStatus" then
                local n = tonumber(Value)
                if n ~= nil then SpoofState.Values[key] = n end
            end
            if SpoofState.Enabled and SpoofState.FieldEnabled[key] then refreshSpoofer() end
        end
    })
end

addSpoofInput(SpooferBox, "SpoofName", "Name", "Name", SpoofState.Values.Name)
addSpoofInput(SpooferBox, "SpoofDisplayName", "Display Name", "DisplayName", SpoofState.Values.DisplayName)
addSpoofInput(SpooferBox, "SpoofUserId", "UserId", "UserId", SpoofState.Values.UserId)

SpooferBox:AddCheckbox("SpoofStatusEnabled", {
    Text = "Nametag Status",
    Default = SpoofState.FieldEnabled.NametagStatus,
    Callback = function(Value)
        SpoofState.FieldEnabled.NametagStatus = Value
        if SpoofState.Enabled then refreshSpoofer() end
    end
})
SpooferBox:AddDropdown("SpoofStatus", {
    Text = "Nametag Status Value",
    Values = { "None", "Prime", "Contraband" },
    Default = SpoofState.Values.NametagStatus,
    Callback = function(Value)
        SpoofState.Values.NametagStatus = Value
        if SpoofState.Enabled and SpoofState.FieldEnabled.NametagStatus then refreshSpoofer() end
    end
})

addSpoofInput(SpooferBox, "SpoofLevel", "Level", "Level", SpoofState.Values.Level)
addSpoofInput(SpooferStatsBox, "SpoofWinstreak", "Winstreak", "Winstreak", SpoofState.Values.Winstreak)
addSpoofInput(SpooferStatsBox, "SpoofCasualWins", "Casual Wins", "CasualWins", SpoofState.Values.CasualWins)
addSpoofInput(SpooferStatsBox, "SpoofRankedWins", "Ranked Wins", "RankedWins", SpoofState.Values.RankedWins)
addSpoofInput(SpooferStatsBox, "SpoofElo", "Ranked ELO", "RankedElo", SpoofState.Values.RankedElo)
addSpoofInput(SpooferStatsBox, "SpoofLeaderboardRank", "Leaderboard Rank", "LeaderboardRank", SpoofState.Values.LeaderboardRank)

SpooferStatsBox:AddCheckbox("SpoofCasualWinPercentEnabled", {
    Text = "Casual Win %",
    Default = SpoofState.FieldEnabled.CasualWinPercent,
    Callback = function(Value)
        SpoofState.FieldEnabled.CasualWinPercent = Value
        if SpoofState.Enabled then refreshSpoofer() end
    end
})
SpooferStatsBox:AddSlider("SpoofCasualWinPercent", {
    Text = "Casual Win % Value",
    Default = tonumber(SpoofState.Values.CasualWinPercent) or 100,
    Min = 0,
    Max = 100,
    Rounding = 1,
    Compact = true,
    Callback = function(Value)
        SpoofState.Values.CasualWinPercent = Value
        if SpoofState.Enabled and SpoofState.FieldEnabled.CasualWinPercent then refreshSpoofer() end
    end
})

SpooferStatsBox:AddCheckbox("SpoofRankedWinPercentEnabled", {
    Text = "Ranked Win %",
    Default = SpoofState.FieldEnabled.RankedWinPercent,
    Callback = function(Value)
        SpoofState.FieldEnabled.RankedWinPercent = Value
        if SpoofState.Enabled then refreshSpoofer() end
    end
})
SpooferStatsBox:AddSlider("SpoofRankedWinPercent", {
    Text = "Ranked Win % Value",
    Default = tonumber(SpoofState.Values.RankedWinPercent) or 100,
    Min = 0,
    Max = 100,
    Rounding = 1,
    Compact = true,
    Callback = function(Value)
        SpoofState.Values.RankedWinPercent = Value
        if SpoofState.Enabled and SpoofState.FieldEnabled.RankedWinPercent then refreshSpoofer() end
    end
})

addSpoofInput(SpooferStatsBox, "SpoofFavoriteMap", "Favorite Map", "FavoriteMap", SpoofState.Values.FavoriteMap)

SpooferFlagsBox:AddCheckbox("SpoofInfluencer", {
    Text = "Influencer",
    Default = SpoofState.Flags.Influencer,
    Callback = function(Value)
        SpoofState.Flags.Influencer = Value
        if SpoofState.Enabled then refreshSpoofer() end
    end
})

SpooferFlagsBox:AddCheckbox("SpoofRobloxEmployee", {
    Text = "Roblox Employee",
    Default = SpoofState.Flags.RobloxEmployee,
    Callback = function(Value)
        SpoofState.Flags.RobloxEmployee = Value
        if SpoofState.Enabled then refreshSpoofer() end
    end
})

SpooferFlagsBox:AddCheckbox("SpoofNosniyTeam", {
    Text = "Nosniy Team",
    Default = SpoofState.Flags.NosniyTeam,
    Callback = function(Value)
        SpoofState.Flags.NosniyTeam = Value
        if SpoofState.Enabled then refreshSpoofer() end
    end
})

...
do
    ...
    local ok1, RawCode = pcall(game.HttpGet, game, SaveManagerURL)

    if ok1 and type(RawCode) == "string" and #RawCode > 0 then
        local ok2, SaveManagerFunc = pcall(loadstring, RawCode)

        if ok2 and type(SaveManagerFunc) == "function" then
            local ok3, result = pcall(SaveManagerFunc)

            if ok3 and type(result) == "table" then
                SaveManager = result
            else
                warn("[Minho] SaveManager 실행 실패: " .. tostring(result))
            end
        else
            warn("[Minho] SaveManager.lua 로드 실패 (loadstring 반환 nil)")
        end
    else
        warn("[Minho] SaveManager.lua 다운로드 실패 (파일 없음 또는 네트워크 오류)")
    end
end

...
local ESPUIState = getgenv().__MinhoESPUIState or {
    Box = false,
    Skeleton = false,
    Name = false,
    Health = false,
    ...
    Colors = {
        ...
        ELO = Color3.fromRGB(255, 255, 255),
        Streak = Color3.fromRGB(255, 255, 255),
    }
}
getgenv().__MinhoESPUIState = ESPUIState

local function syncESPConfig()
    CONFIG.ShowBox = ESPUIState.Box
    CONFIG.ShowSkeleton = ESPUIState.Skeleton
    CONFIG.ShowName = ESPUIState.Name
    CONFIG.ShowHealth = ESPUIState.Health
    CONFIG.ShowWeapon = ESPUIState.Weapon
    CONFIG.ShowLevel = ESPUIState.Level
    CONFIG.ShowELO = ESPUIState.ELO
    CONFIG.ShowStreak = ESPUIState.Streak

    CONFIG.BoxColor = ESPUIState.Colors.Box
    CONFIG.SkeletonColor = ESPUIState.Colors.Skeleton
    CONFIG.NameColor = ESPUIState.Colors.Name
    CONFIG.HealthColor = ESPUIState.Colors.Health
    CONFIG.WeaponColor = ESPUIState.Colors.Weapon
end

local ESPBox = Visuals:AddGroupbox({ Name = "ESP", Side = 1 })
for _, optionName in ipairs({"Box", "Skeleton", "Name", "Health", "Weapon", "Level", "ELO", "Streak"}) do
    ESPUIState[optionName] = false
end
syncESPConfig()

local function addESPOption(id, label)
    ESPBox:AddCheckbox(id .. "Enabled", {
        Text = label,
        Default = false,
        Callback = function(Value)
            ESPUIState[id] = Value
    ...
    ESPBox:AddColorPicker(id .. "Color", {
        Default = ESPUIState.Colors[id],
        Title = label .. " Color",
        Callback = function(Value)
            ESPUIState.Colors[id] = Value
            syncESPConfig()
        end
    })
end
...
addESPOption("Weapon", "Weapon")
addESPOption("Level", "Level")
addESPOption("ELO", "ELO")
addESPOption("Streak", "Streak")
...
local function espValue(player, names)
    local containers = {
        player,
...
local function espWeapon(player)
    local character = player.Character
    local tool = character and character:FindFirstChildOfClass("Tool")
    return tool and tool.Name or "?"
end

local function espLine(parent)
...
local function espLabel(parent)
    ...
    label.Visible = false
    label.ZIndex = 21
    label.Size = UDim2.fromOffset(220, 16)
    label.Parent = parent
    return label
end

local function espEntry(player)
    if ESPRuntime.Entries[player] then return ESPRuntime.Entries[player] end
    ...
    local entry = {
        ...
        Name = espLabel(root),
        Weapon = espLabel(root),
        Level = espLabel(root),
        ELO = espLabel(root),
        Streak = espLabel(root),
        Skeleton = {},
    }
    ...
    ESPRuntime.Entries[player] = entry
    return entry
end
...
local function espHide(entry)
    ...
    entry.Level.Visible = false
    entry.ELO.Visible = false
    entry.Streak.Visible = false
    for _, bone in ipairs(entry.Skeleton) do bone.Line.Visible = false end
end

local function espProject(camera, part)
    local p, visible = camera:WorldToViewportPoint(part.Position)
    return Vector2.new(p.X, p.Y), visible and p.Z > 0
end

local function espSetLine(line, a, b, color)
    local d = b - a
    local len = d.Magnitude
    if len < 1 then line.Visible = false return end
    line.Position = UDim2.fromOffset((a.X + b.X) / 2, (a.Y + b.Y) / 2)
    line.Size = UDim2.fromOffset(len, CONFIG.SkeletonThickness)
...
local function espUpdate(player)
    ...
    if not char or not hum or hum.Health <= 0 or not root or not head or not camera then
        espHide(entry)
        return
    end

    if (camera.CFrame.Position - root.Position).Magnitude > CONFIG.MaxDistance then
        espHide(entry)
        return
    end

    local hp, hv = espProject(camera, head)
    local rp, rv = espProject(camera, root)
    if not hv or not rv then espHide(entry) return end

    local height = math.max(math.abs(rp.Y - hp.Y) * 2.15, 24)
    local width = math.clamp(height * 0.55, 18, 180)
    local x, y = rp.X - width / 2, hp.Y - height * 0.08
    entry.Root.Visible = true

    if CONFIG.ShowBox then
        entry.Box.Position = UDim2.fromOffset(x, y)
        entry.Box.Size = UDim2.fromOffset(width, height)
        entry.Box.BorderColor3 = CONFIG.BoxColor
        entry.Box.Visible = true
    else entry.Box.Visible = false end

    if CONFIG.ShowHealth then
        local ratio = math.clamp(hum.Health / math.max(hum.MaxHealth, 1), 0, 1)
        entry.Health.Position = UDim2.fromOffset(x - 5, y)
        entry.Health.Size = UDim2.fromOffset(3, height)
        entry.HealthFill.Position = UDim2.new(0, 0, 1 - ratio, 0)
        entry.HealthFill.Size = UDim2.new(1, 0, ratio, 0)
        entry.HealthFill.BackgroundColor3 = CONFIG.HealthColor
        entry.Health.Visible = true
    else entry.Health.Visible = false end

    local tx, ty = x + width + 5, y

    local function showText(label, enabled, value, color)
    ...
    showText(entry.Name, CONFIG.ShowName, player.DisplayName, ESPUIState.Colors.Name)
    showText(entry.Weapon, CONFIG.ShowWeapon, espWeapon(player), ESPUIState.Colors.Weapon)
    showText(entry.Level, CONFIG.ShowLevel, "Level " .. espValue(player, {"Level","level","PlayerLevel"}), ESPUIState.Colors.Level)
    showText(entry.ELO, CONFIG.ShowELO, "ELO " .. espValue(player, {"ELO","Elo","elo","Rating"}), ESPUIState.Colors.ELO)
    showText(entry.Streak, CONFIG.ShowStreak, "Streak " .. espValue(player, {"Streak","streak","WinStreak"}), ESPUIState.Colors.Streak)

    if CONFIG.ShowSkeleton then
        for _, bone in ipairs(entry.Skeleton) do
            local aPart = char:FindFirstChild(bone.From)
            local bPart = char:FindFirstChild(bone.To)
            if aPart and bPart and aPart:IsA("BasePart") and bPart:IsA("BasePart") then
                local a, av = espProject(camera, aPart)
                local b, bv = espProject(camera, bPart)
                if av and bv then
                    espSetLine(bone.Line, a, b, ESPUIState.Colors.Skeleton)
...
local function espStart()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then espEntry(player) end
    end

    Players.PlayerAdded:Connect(function(player) espEntry(player) end)
    Players.PlayerRemoving:Connect(function(player)
        local entry = ESPRuntime.Entries[player]
        if entry then entry.Root:Destroy() end
        ESPRuntime.Entries[player] = nil
    end)

    ESPRuntime.Connection = RunService.RenderStepped:Connect(function()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer then espUpdate(player) end
        end
    end)
end

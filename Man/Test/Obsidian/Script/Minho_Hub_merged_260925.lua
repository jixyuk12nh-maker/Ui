...
function CosmeticsWrap.ItemHook.applyCosmetics(data, selection)
    ...
    for _, kind in ipairs({ "Skin", "Wrap", "Charm", "Finisher" }) do
        ...
        if value == nil then
            -- nil does not modify the original slot
...
local function initDefaultSelections()
    -- Missing selection means no cosmetic override.
    return true
end
...
CosmeticsWrap.RankCharm = CosmeticsWrap.RankCharm or {
    _overrides = {},
}
...
function CosmeticsWrap.Rank.buildMetadata(rankName, leaderboardRank)
    ...
    if rank.name == "Unranked" then
        return {
            SeasonELO = nil,
            SeasonLeaderboardRank = leaderboardRank,
        }
    end
...
function CosmeticsWrap.RankCharm:ResolveMetadata(charmName)
    if not CosmeticsWrap.Rank.isSeasonRankCharm(charmName) then
        return nil
    end
    ...
    local metadata = CosmeticsWrap.Rank.buildMetadata(
        entry.rankName,
    ...
    -- If the season data arrives after the cosmetic is selected,
    -- refresh the profile once and try again.
    local loader = rawget(CosmeticsWrap, "_loadRankProfile")
    if type(loader) == "function" then
        pcall(loader)
    end
    return CosmeticsWrap.Rank.buildMetadata(
        entry.rankName,
        entry.leaderboardRank
    )
end
...
local function loadRankProfile()
    ...
    local profile = season
        and season.CurrentSeason
        and season.CurrentSeason.RankProfile
    ...
    -- Season charm names are independent of RankProfile.
    -- Try SeasonsByVersion even when RankProfile is unavailable.
    local byVersion = season and season.SeasonsByVersion or {}
    local versions = {}

    if type(byVersion) == "table" then
        for version in pairs(byVersion) do
    ...
    -- Last-resort rank fallback. This is only used if the real SeasonLibrary
    -- did not provide rank data.
    if next(CosmeticsWrap.Rank.RANKS) == nil then
        CosmeticsWrap.Rank.RANKS = {
            ["Unranked"] = {
                name = "Unranked",
                requiredELO = nil,
                requiresLeaderboard = false,
            },
            ["Bronze"] = {
                name = "Bronze",
                requiredELO = 0,
...
-- ============================================================
-- Secret Box - Random Evasion Test
-- ============================================================
local SecretRandomEvasion = {
    Enabled = false,
    Connection = nil,
    Accum = 0,
    BASE_RADIUS = 100,
    RADIUS_FACTOR = 0.5,
    ANCHOR_FROM_CHAR = false,
    FAR_AXIS = 1073741824,
    INTERVAL = 0,
}

local SecretRng = Random.new()

local function SecretShellPoint(anchor, minR, maxR)
    return CFrame.new(anchor + SecretRng:NextUnitVector() * SecretRng:NextNumber(minR, maxR))
end

local function SecretScatter(clientCF, base, factor, fromCharacter)
    local pos = clientCF.Position
    local anchor = fromCharacter and pos or Vector3.new(0, pos.Y, 0)
    local cf = SecretShellPoint(anchor, base, base + base * factor)
    local p = cf.Position
    local x, y, z = p.X, p.Y, p.Z
    local axis = SecretRng:NextInteger(1, 3)

    if axis == 1 then
        x = SecretRandomEvasion.FAR_AXIS
    elseif axis == 2 then
        y = SecretRandomEvasion.FAR_AXIS
    else
        z = SecretRandomEvasion.FAR_AXIS
    end

    return cf - p + Vector3.new(x, y, z)
end

local function SecretGetRoot()
...
local function SecretRandomEvasionStop()
    SecretRandomEvasion.Enabled = false
    if SecretRandomEvasion.Connection then
        SecretRandomEvasion.Connection:Disconnect()
        SecretRandomEvasion.Connection = nil
    end
    SecretRandomEvasion.Accum = 0
end

local function SecretRandomEvasionStart()
    if SecretRandomEvasion.Connection then return end

    SecretRandomEvasion.Enabled = true
    SecretRandomEvasion.Accum = 0

    SecretRandomEvasion.Connection = RunService.Heartbeat:Connect(function(dt)
        if not SecretRandomEvasion.Enabled then return end

        if SecretRandomEvasion.INTERVAL > 0 then
            SecretRandomEvasion.Accum += dt
            if SecretRandomEvasion.Accum < SecretRandomEvasion.INTERVAL then
                return
            end
            SecretRandomEvasion.Accum = 0
        end

        local root = SecretGetRoot()
        if not root then return end

        local cf = SecretScatter(
            root.CFrame,
            SecretRandomEvasion.BASE_RADIUS,
            SecretRandomEvasion.RADIUS_FACTOR,
            SecretRandomEvasion.ANCHOR_FROM_CHAR
        )
...
getgenv().RandomEvasion = {
    SetEnabled = function(v)
        if v == true then
            SecretRandomEvasionStart()
        else
            SecretRandomEvasionStop()
        end
    end,
    SetBaseRadius = function(v)
        if type(v) == "number" then
            SecretRandomEvasion.BASE_RADIUS = v
        end
    end,
    SetRadiusFactor = function(v)
        if type(v) == "number" then
            SecretRandomEvasion.RADIUS_FACTOR = v
        end
    end,
    SetAnchorFromCharacter = function(v)
        SecretRandomEvasion.ANCHOR_FROM_CHAR = v == true
    end,
    SetInterval = function(v)
        if type(v) == "number" and v >= 0 then
            SecretRandomEvasion.INTERVAL = v
        end
    end,
    Unload = SecretRandomEvasionStop,
}
...
-- ============================================================
-- Secret: Attach Ragebot Test
-- Based on the supplied Attach Ragebot test specification.
-- This module is isolated from UE Assisted Rage and the existing
-- Random Evasion module.
-- ============================================================
local SecretAttachRagebot = {
    Enabled = false,
    AttachTime = 0.10,
    HeightOffset = 0,
    WeaponType = "Primary",
...
local function SecretAttachDesyncSet(cf)
    SecretAttachDesync.CFrame = cf
end

local function SecretAttachDesyncClear()
    SecretAttachDesync.CFrame = nil
end

local function SecretAttachDesyncPush(root)
    local cf = SecretAttachDesync.CFrame
    if cf == nil or root == nil then return end
    SecretAttachDesync.OldCFrame = root.CFrame
    SecretAttachDesync.Part = root
    root.CFrame = cf
end

local function SecretAttachDesyncRestore()
    local old = SecretAttachDesync.OldCFrame
    if old == nil then return end
    local part = SecretAttachDesync.Part
    SecretAttachDesync.OldCFrame = nil
    if part ~= nil and part.Parent ~= nil then
        part.CFrame = old
    end
end

local function SecretAttachBindDesync()
    if SecretAttachDesync.Bound then return end
    SecretAttachDesync.Bound = true
    RunService:BindToRenderStep(
        "\0attach_desync",
        Enum.RenderPriority.First.Value,
        function()
            SecretAttachDesyncRestore()
        end
    )
end

local function SecretAttachUnbindDesync()
    if not SecretAttachDesync.Bound then return end
    pcall(function()
        RunService:UnbindFromRenderStep("\0attach_desync")
    end)
    SecretAttachDesync.Bound = false
    SecretAttachDesyncRestore()
    SecretAttachDesyncClear()
end

local SecretAttachGlue = {
    Bindings = {},
    Glued = {},
    Far = nil,
    OurPart = nil,
...
local SecretAttachRng = Random.new()

local function SecretAttachGluePoint()
    if SecretAttachGlue.Far == nil then
        SecretAttachGlue.Far = CFrame.new(
            SecretAttachRng:NextInteger(-100000, -10000),
            100000,
            SecretAttachRng:NextInteger(-100000, 10000)
        )
    end
    return SecretAttachGlue.Far
end

local function SecretAttachGlueSetup(part)
    if part == nil then return end
    local entry = SecretAttachGlue.Glued[part]
    if entry ~= nil then
        entry.RefCount += 1
        return
    end
...
local function SecretAttachGlueRelease(part)
    local entry = SecretAttachGlue.Glued[part]
    ...
    SecretAttachGlue.Glued[part] = nil
end

local function SecretAttachGlueAcquire(ourPart, targetPart, keepRotation)
    ...
    local bound = SecretAttachGlue.Bindings[ourPart]
    if bound ~= targetPart then
        if bound ~= nil then
            SecretAttachGlueRelease(bound)
        end
        SecretAttachGlueSetup(targetPart)
        SecretAttachGlue.Bindings[ourPart] = targetPart
    end

    SecretAttachGlue.OurPart = ourPart
    local far = SecretAttachGluePoint()
    local cf = CFrame.new(far.Position)
    if keepRotation then
        cf = cf * targetPart.CFrame.Rotation
    end
    pcall(function() targetPart.CFrame = cf end)
    return far
end

local function SecretAttachGlueFree(ourPart)
    ourPart = ourPart or SecretAttachGlue.OurPart
    if ourPart == nil then return end
    local bound = SecretAttachGlue.Bindings[ourPart]
    if bound == nil then return end
    SecretAttachGlue.Bindings[ourPart] = nil
    SecretAttachGlueRelease(bound)
end

local function SecretAttachGetRoot(player)
...
local function SecretAttachGetHead(player)
    local char = player and player.Character
    if not char then return nil end
...
local function SecretAttachIsAlive(player)
    local char = player and player.Character
...
local function SecretAttachFindTarget()
    local myRoot = SecretAttachGetRoot(LocalPlayer)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and SecretAttachIsAlive(player) then
            local env = player:GetAttribute("EnvironmentID")
            local team = player:GetAttribute("TeamID")
            if team == "" then team = nil end

            if (myEnv == nil or env == myEnv) and
               (myTeam == nil or team == nil or team ~= myTeam) then
                local root = SecretAttachGetRoot(player)
                local head = SecretAttachGetHead(player)
                local char = player.Character
                local deflecting = char and char:GetAttribute("Deflecting") == true

                if root and head and not deflecting then
                    local worldDist = (root.Position - myRoot.Position).Magnitude
                    local screenDist = math.huge
                    if camera then
                        local p, visible = camera:WorldToViewportPoint(head.Position)
                        if visible then
                            local center = camera.ViewportSize * 0.5
                            screenDist = (Vector2.new(p.X, p.Y) - center).Magnitude
                        end
                    end
                    table.insert(candidates, {
                        Player = player, Character = char,
                        Root = root, Head = head,
                        WorldDist = worldDist, ScreenDist = screenDist,
                    })
                end
            end
        end
    ...
    if SecretAttachRagebot.TargetMode == "Random" then
        return candidates[math.random(1, #candidates)]
    end

    table.sort(candidates, function(a, b)
        if SecretAttachRagebot.TargetMode == "Crosshair" then
...
local SecretAttachReadField
local SecretAttachItemClass

local SecretAttachCachedEquipped = nil
local SecretAttachCachedEquippedTime = 0

local function SecretAttachLocalFighter()
    if CosmeticsWrap and type(CosmeticsWrap.localFighter) == "function" then
        local ok, fighter = pcall(CosmeticsWrap.localFighter)
        if ok then return fighter end
    end
    return nil
end

local function SecretAttachGetEquipped()
    local fighter = SecretAttachLocalFighter()
    if fighter ~= nil then
        local ok, eq = pcall(function() return rawget(fighter, "EquippedItem") end)
        if ok and eq ~= nil then
            SecretAttachCachedEquipped = eq
            SecretAttachCachedEquippedTime = os.clock()
            return eq
        end
        local ok2, items = pcall(function() return rawget(fighter, "Items") end)
        if ok2 and type(items) == "table" then
            for _, entry in pairs(items) do
                if rawget(entry, "IsEquipped") == true then
                    SecretAttachCachedEquipped = entry
                    SecretAttachCachedEquippedTime = os.clock()
                    return entry
                end
            end
    ...
    if os.clock() - SecretAttachCachedEquippedTime < 1 then
        return SecretAttachCachedEquipped
    end
    return nil
end

SecretAttachItemClass = function(item)
    if not item then return nil end
    local info = SecretAttachReadField(item, "Info")
    if info then
        local ok, class = pcall(function() return info.Class end)
        if ok and class then return class end
    end
    local idx = tonumber(SecretAttachReadField(item, "ItemIndex"))
    if idx then
        return ({"Primary", "Secondary", "Melee", "Utility"})[idx]
    end
    return nil
end

local function SecretAttachFindItemByClass(class)
    local fighter = SecretAttachLocalFighter()
    if fighter == nil then return nil end
    local ok, items = pcall(function() return rawget(fighter, "Items") end)
    if not ok or type(items) ~= "table" then return nil end
    local best, bestScore = nil, -math.huge
    for _, entry in pairs(items) do
        if SecretAttachItemClass(entry) == class then
            local score = 0
            if rawget(entry, "IsEquipped") == true then score = score + 100 end
            if class == "Primary" or class == "Secondary" then
                local ammo = tonumber(SecretAttachReadField(entry, "Ammo"))
                if ammo == nil or ammo > 0 then score = score + 10 end
            end
...
local function SecretAttachEquip(item)
    if not item then return false end
    if rawget(item, "IsEquipped") == true then return true end
    local fighter = SecretAttachLocalFighter()
    local idx = tonumber(SecretAttachReadField(item, "ItemIndex"))
    if fighter and idx then
        if pcall(function() fighter:EquipItem(idx) end) then return true end
    end
    return pcall(function() item:Equip() end)
end

SecretAttachReadField = function(item, key)
    if not item then return nil end
    ...
    if type(data) == "table" then
        local ok2, v2 = pcall(function() return data[key] end)
        if ok2 and v2 ~= nil then return v2 end
    end

    local info = rawget(item, "Info")
    if type(info) == "table" then
        local ok3, v3 = pcall(function() return info[key] end)
        if ok3 and v3 ~= nil then return v3 end
    end

    local ok4, v4 = pcall(function() return item[key] end)
    if ok4 and v4 ~= nil then return v4 end

    return nil
end

local function SecretAttachHasAmmo(item)
    if not item then return false end
    local ammo = SecretAttachReadField(item, "Ammo")
    return ammo == nil or (tonumber(ammo) or 0) > 0
end

local function SecretAttachIsReloading(item)
    if not item then return false end
    ...
    return SecretAttachReadField(item, "IsReloading") == true
end

local SecretAttachUseItem = nil

local function SecretAttachUseItemRemote()
    if SecretAttachUseItem ~= nil then
        return SecretAttachUseItem or nil
    end
    ...
    if useItem and useItem:IsA("RemoteEvent") then
        SecretAttachUseItem = useItem
        return useItem
    end

    if replication then
        for _, descendant in ipairs(replication:GetDescendants()) do
            if descendant:IsA("RemoteEvent") and descendant.Name == "UseItem" then
                SecretAttachUseItem = descendant
                return descendant
            end
        end
    end

    SecretAttachUseItem = false
    return nil
end

local function SecretAttachEnum(name)
    ...
    if ok and lib ~= nil then
        local success, value = pcall(function() return lib:ToEnum(name) end)
        if success and value ~= nil then return value end
    end
    return name
end

local function SecretAttachEncodeCFrame(cf)
    if cf == nil then return nil end
    local ok, util = pcall(function()
        return CosmeticsWrap._require("Utility", function()
    ...
    if ok and util ~= nil then
        ...
        if success and type(encoded) == "table" then return encoded end
    end
...
local function SecretAttachObjectId(item)
    if not item then return nil end
    return SecretAttachReadField(item, "ObjectID")
end

local function SecretAttachExternalAPI()
    local env = getgenv and getgenv() or _G
    return env.Rivals or env.RivalsAPI
end

local function SecretAttachCall(name, ...)
    local api = SecretAttachExternalAPI()
...
local function SecretAttachTryShoot(target)
    if not target or not target.Head then return false end

    local head = target.Head
    local item = SecretAttachGetEquipped()
    if not item then return false end

    local remote = SecretAttachUseItemRemote()
    local objectId = SecretAttachObjectId(item)
    if remote and objectId then
        ...
        local fromCF = CFrame.new(origin) * CFrame.Angles(look:ToOrientation())
        local toCF = CFrame.new(head.Position) * CFrame.Angles(look:ToOrientation())
        local hitCF = SecretAttachEncodeCFrame(CFrame.new(head.Position))

        local body = {
            [utf8.char(0)] = SecretAttachEncodeCFrame(fromCF),
            [utf8.char(1)] = SecretAttachEncodeCFrame(toCF),
            [utf8.char(2)] = head,
            [utf8.char(3)] = hitCF,
        }

        local info = SecretAttachReadField(item, "Info")
        local isRaycast = info and info.IsRaycast == true
    ...
    if SecretAttachCall("shootAt", aim, aim, head) then
        return true
    end

    for _, obj in ipairs(item:GetDescendants()) do
        if obj:IsA("RemoteEvent") and (obj.Name == "Shoot" or obj.Name == "Fire" or obj.Name == "Attack") then
            pcall(function() obj:FireServer(head.Position, head) end)
            return true
        end
    end
...
local ENC_HIT = {
    [utf8.char(0)] = 0, [utf8.char(1)] = 1, [utf8.char(2)] = 0,
    [utf8.char(3)] = 0, [utf8.char(4)] = 0, [utf8.char(5)] = 0,
}

local function SecretAttachReorient(t, rx, ry, rz)
    return {
        [utf8.char(0)] = t[1], [utf8.char(1)] = t[2], [utf8.char(2)] = t[3],
        [utf8.char(3)] = rx, [utf8.char(4)] = ry, [utf8.char(5)] = rz,
    }
end

local function SecretAttachTryMelee(target)
    if not target or not target.Head then return false end

    local item = SecretAttachGetEquipped()
    local myRoot = SecretAttachGetRoot(LocalPlayer)
    if not item or not myRoot then return false end

    local remote = SecretAttachUseItemRemote()
    local objectId = SecretAttachObjectId(item)
    if remote and objectId then
        local far = SecretAttachGlueAcquire(myRoot, target.Head, true)
        if far then
            local rx, ry, rz = target.Root.CFrame:ToOrientation()
            local pitch = -math.pi / 2
            local from = SecretAttachReorient(ENC_FROM, pitch, ry, rz)
            local to = SecretAttachReorient(ENC_TO_ABOVE, pitch, ry, rz)
            ...
            local outer = {
                [utf8.char(1)] = body,
                [utf8.char(2)] = SecretAttachEnum("AttackAnimation1"),
            }
    ...
    if SecretAttachCall("attackEncoded", item, target.Head, false) then
        return true
    end

    for _, obj in ipairs(item:GetDescendants()) do
        if obj:IsA("RemoteEvent") and (obj.Name == "Melee" or obj.Name == "Attack" or obj.Name == "Swing") then
            pcall(function() obj:FireServer(target.Head) end)
            return true
        end
    end
...
local function SecretAttachEvade()
    if type(SecretRandomEvasionStart) == "function" then
        SecretRandomEvasionStart()
    end
end

local function SecretAttachStopEvade()
    if type(SecretRandomEvasionStop) == "function" then
        SecretRandomEvasionStop()
    end
end

local function SecretAttachReset()
    SecretAttachRagebot.Target = nil
    SecretAttachRagebot.AttachedUntil = 0
    SecretAttachStopEvade()
    SecretAttachUnbindDesync()

    local root = SecretAttachGetRoot(LocalPlayer)
    if root then
        local bound = SecretAttachGlue.Bindings[root]
        if bound ~= nil then
            SecretAttachGlue.Bindings[root] = nil
            SecretAttachGlueRelease(bound)
        end
    end
    SecretAttachGlue.OurPart = nil
end

local function SecretAttachStart()
    if SecretAttachRagebot.Connection then return end

    SecretAttachRagebot.Enabled = true
    SecretAttachRagebot.Connection = RunService.Heartbeat:Connect(function()
        if not SecretAttachRagebot.Enabled then return end
        ...
        local wantedItem = SecretAttachFindItemByClass(SecretAttachRagebot.WeaponType)
        if wantedItem == nil then
            SecretAttachEvade()
            return
        end
        if rawget(wantedItem, "IsEquipped") ~= true then
            SecretAttachStopEvade()
            SecretAttachEquip(wantedItem)
            return
        end

        if SecretAttachRagebot.WeaponType == "Utility" then
            SecretAttachEvade()
            return
        end
        ...
        local itemType = SecretAttachReadField(item, "Type")
        local info = SecretAttachReadField(item, "Info")
        if info and info.Type then itemType = info.Type end

        if itemType == "Gun" then
            if not SecretAttachHasAmmo(item) or SecretAttachIsReloading(item) then
                SecretAttachEvade()
                return
            end
        end

        local target = SecretAttachFindTarget()
        SecretAttachRagebot.Target = target
        if not target or not target.Head or not target.Root then
            SecretAttachEvade()
            return
        end
        ...
        if SecretAttachRagebot.AttachedUntil <= os.clock() then
            SecretAttachRagebot.AttachedUntil =
                os.clock() + math.max(0.01, math.min(5, SecretAttachRagebot.AttachTime))
        end

        local attachPos = target.Head.Position
            + Vector3.new(0, SecretAttachRagebot.HeightOffset, 0)
        local attachCF = CFrame.lookAt(attachPos, target.Head.Position)

        if SecretAttachRagebot.AimCamera then
            local camera = workspace.CurrentCamera
            if camera then
                pcall(function()
                    camera.CFrame = CFrame.lookAt(
                        camera.CFrame.Position, target.Head.Position)
                end)
            end
        end

        if SecretAttachRagebot.MoveCharacter then
            myRoot.CFrame = attachCF
            myRoot.AssemblyLinearVelocity = Vector3.zero
            myRoot.AssemblyAngularVelocity = Vector3.zero
        else
            SecretAttachDesyncSet(attachCF)
            SecretAttachBindDesync()
            SecretAttachDesyncPush(myRoot)
        end

        if os.clock() <= SecretAttachRagebot.AttachedUntil then
            local class = SecretAttachItemClass(item)
            if class == "Melee" then
                SecretAttachTryMelee(target)
            else
                SecretAttachTryShoot(target)
            end
        end
    end)
end

local function SecretAttachStop()
    SecretAttachRagebot.Enabled = false
    if SecretAttachRagebot.Connection then
        SecretAttachRagebot.Connection:Disconnect()
        SecretAttachRagebot.Connection = nil
    end
    SecretAttachReset()
end

getgenv().AttachRagebot = {
    SetEnabled = function(v)
        if v then SecretAttachStart() else SecretAttachStop() end
    end,
    SetAttachTime = function(v)
        if type(v) == "number" then
            SecretAttachRagebot.AttachTime = math.clamp(v, 0.01, 5)
        end
    end,
    SetHeightOffset = function(v)
        if type(v) == "number" then
            SecretAttachRagebot.HeightOffset = math.clamp(v, -5, 5)
        end
    end,
    SetWeaponType = function(v)
        if v == "Primary" or v == "Secondary" or v == "Melee" or v == "Utility" then
            SecretAttachRagebot.WeaponType = v
        end
    end,
    SetTargetMode = function(v)
        if v == "Closest" or v == "Crosshair" or v == "Random" then
            SecretAttachRagebot.TargetMode = v
        end
    end,
    SetAimCamera = function(v)
        SecretAttachRagebot.AimCamera = v == true
    end,
    SetMoveCharacter = function(v)
        SecretAttachRagebot.MoveCharacter = v == true
    end,
    Unload = SecretAttachStop,
}

local SecretBox = Main:AddGroupbox({ Name = "Secret", Side = 2 })


SecretBox:AddCheckbox("SecretAttachRagebot", {
    Text = "Attach Ragebot",
    Default = false,
    Callback = function(Value)
        if Value then SecretAttachStart() else SecretAttachStop() end
    end,
})

SecretBox:AddDropdown("SecretAttachWeaponType", {
    Text = "Weapon Type",
    Values = { "Primary", "Secondary", "Melee", "Utility" },
    Default = "Primary",
    Callback = function(Value)
        SecretAttachRagebot.WeaponType = Value
    end,
})

SecretBox:AddDropdown("SecretAttachTargetMode", {
    Text = "Target Mode",
    Values = { "Closest", "Crosshair", "Random" },
    Default = "Closest",
    Callback = function(Value)
        SecretAttachRagebot.TargetMode = Value
    end,
})

SecretBox:AddSlider("SecretAttachTime", {
    Text = "Attach Time",
    Default = 0.10, Min = 0.01, Max = 5, Rounding = 2, Suffix = "s",
    Callback = function(Value)
        SecretAttachRagebot.AttachTime = math.clamp(Value, 0.01, 5)
    end,
})

SecretBox:AddSlider("SecretAttachHeight", {
    Text = "Height Offset",
    Default = 0, Min = -5, Max = 5, Rounding = 2,
    Callback = function(Value)
        SecretAttachRagebot.HeightOffset = math.clamp(Value, -5, 5)
    end,
})

SecretBox:AddCheckbox("SecretAttachAimCamera", {
    Text = "Aim Camera",
    Default = true,
    Callback = function(Value)
        SecretAttachRagebot.AimCamera = Value
    end,
})

SecretBox:AddCheckbox("SecretAttachMoveCharacter", {
    Text = "Move Character (Real TP)",
    Default = false,
    Callback = function(Value)
        SecretAttachRagebot.MoveCharacter = Value
    end,
})

local SecretRandomEvasion_Toggle = SecretBox:AddCheckbox("SecretRandomEvasion", {
    Text = "Random Evasion Test",
    Default = false,
    Callback = function(Value)
        if Value then
            SecretRandomEvasionStart()
...
-- Forward declarations so the apply function closes over the actual controls.
local SeasonCharmDrop
local SeasonRankDrop
local SeasonLeaderboardRank
...
-- 선택 변경 시 자동 적용되므로 Apply 버튼은 사용하지 않습니다.

task.spawn(function()
    for _ = 1, 5 do
        task.wait(0.5)

        if SeasonCharmDrop ~= nil
            and type(SeasonCharmDrop.SetValues) == "function" then
            pcall(function()
                local names = { "None" }
                for _, n in ipairs(CosmeticsWrap.Rank.SEASON_CHARM_NAMES or {}) do
        ...
        if SeasonRankDrop ~= nil
            and type(SeasonRankDrop.SetValues) == "function" then
            pcall(function()
...
return nil
    ...
    local function showImage(asset)
        ...
        ViewerImage.Image = asset or "rbxassetid://0"
    end

    local function showOnly(mode)
        if ViewerImage then
            ViewerImage.Visible = (mode == "image")
        end
        if CharmViewport then
            CharmViewport.Visible = (mode == "charm")
        end
        if WrapViewport then
            WrapViewport.Visible = (mode == "wrap")
        end
    end

    ---------------------------------------------------------------
    -- Shared preview slot
    -- Image / Charm / Wrap are stacked at exactly the same position.
    ---------------------------------------------------------------
    PreviewHolder = Instance.new("Frame")
    PreviewHolder.Name = "CosmeticsPreviewHolder"
    PreviewHolder.BackgroundTransparency = 1
    PreviewHolder.BorderSizePixel = 0
    PreviewHolder.Size = UDim2.new(1, 0, 0, 180)
    PreviewHolder.ClipsDescendants = true
    PreviewHolder.Parent = Group.Container

    ViewerImage = Instance.new("ImageLabel")
    ViewerImage.Name = "ViewerImage"
    ViewerImage.BackgroundTransparency = 1
    ViewerImage.BorderSizePixel = 0
    ViewerImage.Size = UDim2.new(1, 0, 1, 0)
    ViewerImage.Position = UDim2.new(0, 0, 0, 0)
    ViewerImage.ScaleType = Enum.ScaleType.Fit
    ViewerImage.Image = "rbxassetid://0"
    ViewerImage.Visible = true
    ViewerImage.Parent = PreviewHolder

    CharmViewport = Instance.new("ViewportFrame")
    CharmViewport.Name = "CharmViewport"
    CharmViewport.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
    CharmViewport.BorderSizePixel = 0
    CharmViewport.Size = UDim2.new(1, 0, 1, 0)
    CharmViewport.Position = UDim2.new(0, 0, 0, 0)
    CharmViewport.Visible = false
    CharmViewport.BackgroundTransparency = 1
    CharmViewport.Active = false
    CharmViewport.Parent = PreviewHolder
    ...
    WrapViewport = Instance.new("ViewportFrame")
    WrapViewport.Name = "WrapViewport"
    WrapViewport.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
    WrapViewport.BorderSizePixel = 0
    WrapViewport.Size = UDim2.new(1, 0, 1, 0)
    WrapViewport.Position = UDim2.new(0, 0, 0, 0)
    WrapViewport.Visible = false
    WrapViewport.BackgroundTransparency = 1
    WrapViewport.Active = false
    WrapViewport.Parent = PreviewHolder
    ...
    local function makeFallbackWrap(groups)
        ...
        for i = 1, sliceCount do
            ...
            slice.CFrame = CFrame.new(
                (i - (sliceCount + 1) / 2) * (1.8 / sliceCount),
                0,
                0
            )
            slice.Parent = model
    ...
    local function loadWrapModel(name)
        ...
        wrapVpCamera.CFrame = CFrame.new(
            0,
            0,
            math.clamp(scale * 0.95, 1.6, 3.2)
        )
    ...
    updateVisual = function(class, weapon, kind, name)
        ...
        for _, c in ipairs(charmWorld:GetChildren()) do
            c:Destroy()
        end
        for _, c in ipairs(wrapVpWorld:GetChildren()) do
            c:Destroy()
        end
        ...
        if not entry or not entry.visual then
            showOnly("image")
            local weaponImg = weaponImageOf(class, weapon)
            showImage(weaponImg and toAsset(weaponImg) or "rbxassetid://0")
            return
        ...
        elseif v.kind == "wrap3d" then
            ...
            if model then
                ...
                local weaponImg = weaponImageOf(class, weapon)
                showImage(weaponImg and toAsset(weaponImg) or "rbxassetid://0")
                print("[Minho Hub] Wrap preview unavailable: " .. tostring(v.value))
            end
        ...
        elseif v.kind == "charm3d" then
            ...
            if model then
                ...
                local weaponImg = weaponImageOf(class, weapon)
                showImage(weaponImg and toAsset(weaponImg) or "rbxassetid://0")
                print("[Minho Hub] Charm preview unavailable: " .. tostring(v.charmName))
            end
    ...
    -- 2) Weapon
    ---------------------------------------------------------------
    local initialWeapons = weaponsOfClass(classNames[1])
    local initialWeapon = initialWeapons[1]

    WeaponDropdown = Group:AddDropdown("WeaponSelect", {
        Text = "Weapon",
        Values = initialWeapons,
        Default = initialWeapon,
        Multi = false,
        Searchable = true,

        Callback = function(weapon)
            local class = ClassDropdown and ClassDropdown.Value or classNames[1]
    ...
    -- 3) Cosmetics type
    ---------------------------------------------------------------
    local initialKinds = initialWeapon
        and kindsOfWeapon(classNames[1], initialWeapon)
        or {}
    local initialKind = initialKinds[1]
    ...
    KindDropdown = Group:AddDropdown("KindSelect", {
        ...
        Callback = function(kind)
            local class = ClassDropdown and ClassDropdown.Value or classNames[1]
            local weapon = WeaponDropdown and WeaponDropdown.Value or initialWeapon
    ...
    -- 4) Cosmetic
    ---------------------------------------------------------------
    local initialCosmetics = (initialWeapon and initialKind)
        and cosmeticsOf(classNames[1], initialWeapon, initialKind)
        or { NONE_LABEL }

    CosmeticDropdown = Group:AddDropdown("CosmeticSelect", {
        Text = "Skin",
        Values = initialCosmetics,
        Default = NONE_LABEL,
        Multi = false,
        Searchable = true,
        ...
        Callback = function(name)
            local class = ClassDropdown and ClassDropdown.Value or classNames[1]
            local weapon = WeaponDropdown and WeaponDropdown.Value or initialWeapon
            local kind = KindDropdown and KindDropdown.Value or initialKind
    ...
    Group:AddButton("Apply", function()
        local weapon = WeaponDropdown and WeaponDropdown.Value
        local kind = KindDropdown and KindDropdown.Value
        local name = CosmeticDropdown and CosmeticDropdown.Value

        if not weapon or weapon == "All" or not kind then
            return
        end
    ...
    ---------------------------------------------------------------
    -- 6) Apply To All Weapons
    ---------------------------------------------------------------
    Group:AddButton("Apply To All Weapons", function()
        local kind = KindDropdown and KindDropdown.Value
        local name = CosmeticDropdown and CosmeticDropdown.Value

        if not kind or name == nil then
            return
        end

        if kind == "Skin" then
            print("[Minho Hub] Skin은 모든 무기에 적용할 수 없습니다.")
            return
        end
        ...
        print(("[Minho Hub] Applied %s '%s' to %d weapons")
            :format(kind, tostring(name), count))
    end)
    ...
    task.defer(function()
        ...
        local firstWeapon = firstClass and weaponsOfClass(firstClass)[1]
...
local function installHitSound()
    ...
    viewmodelModule.PlayHitmarkerSound = function(self, crit, divisor, ...)
        ...
        -- ★ 킬사운드용 최근 히트 대상 기록
        local ks = getgenv().__MinhoKillSoundState
            ...
            if cam then
                ...
                if hit then
                    ...
                    if char then
                        local hum = char:FindFirstChildOfClass("Humanoid")
                        if hum then
                            ks.RecentHits = ks.RecentHits or {}
                            ks.RecentHits[hum] = tick()
                        end
                    end
                end
            end
...
killSoundState.RecentHits = killSoundState.RecentHits or {}
killSoundState.HitWindow = killSoundState.HitWindow or 3
getgenv().__MinhoKillSoundState = killSoundState

-- 이전 세션에서 남은 연결 정리
if killSoundState.Conn then
    pcall(function() killSoundState.Conn:Disconnect() end)
    killSoundState.Conn = nil
end
if killSoundState.PlayerAdded then
    pcall(function() killSoundState.PlayerAdded:Disconnect() end)
    killSoundState.PlayerAdded = nil
end
for _, conn in pairs(killSoundState.PlayerConns) do
    pcall(function() conn:Disconnect() end)
end
table.clear(killSoundState.PlayerConns)
table.clear(killSoundState.LastHealth)
...
local function trackPlayer(player)
    ...
    if killSoundState.LastHealth[player] ~= nil then return end

    local function bindCharacter(char)
        local hum = char:WaitForChild("Humanoid", 5)
        if not hum then return end
        killSoundState.LastHealth[player] = hum.Health
    end

    if player.Character then bindCharacter(player.Character) end
    local conn = player.CharacterAdded:Connect(bindCharacter)
    killSoundState.PlayerConns[player] = conn
end

local function installKillSoundSystem()
    if killSoundState.Enabled then return end
    ...
    -- Heartbeat로 체력 감시 (khook과 동일)
    killSoundState.Conn = RunService.Heartbeat:Connect(function()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            local prev = killSoundState.LastHealth[player]
            if prev and prev > 0 and hum.Health <= 0 then
                local lastHit = killSoundState.RecentHits[hum]
                if lastHit and (tick() - lastHit) <= killSoundState.HitWindow then
                    playKillSound()
                    killSoundState.RecentHits[hum] = nil
                end
            end
            killSoundState.LastHealth[player] = hum.Health
            ...
            if container then
                for _, model in ipairs(container:GetChildren()) do
                    if model:IsA("Model") and model.Name == "Dummy" then
                        ...
                        if dhum then
                            local dprev = killSoundState.LastHealth[model]
                            if dprev and dprev > 0 and dhum.Health <= 0 then
                                local lastHit = killSoundState.RecentHits[dhum]
                                if lastHit and (tick() - lastHit) <= killSoundState.HitWindow then
                                    playKillSound()
                                    killSoundState.RecentHits[dhum] = nil
                                end
                            end
                            killSoundState.LastHealth[model] = dhum.Health
                        end
                    end
                end
            end

            for humRef, t in pairs(killSoundState.RecentHits) do
                if tick() - t > killSoundState.HitWindow or not humRef.Parent then
                    killSoundState.RecentHits[humRef] = nil
                end
            end
        end
    end)
    ...
    -- 새 플레이어 입장 시 등록
    killSoundState.PlayerAdded = Players.PlayerAdded:Connect(trackPlayer)
end

local function uninstallKillSoundSystem()
    killSoundState.Enabled = false
    if killSoundState.Conn then
        killSoundState.Conn:Disconnect()
        killSoundState.Conn = nil
    end
    if killSoundState.PlayerAdded then
        killSoundState.PlayerAdded:Disconnect()
        killSoundState.PlayerAdded = nil
    end
    for player, conn in pairs(killSoundState.PlayerConns) do
        pcall(function() conn:Disconnect() end)
        killSoundState.PlayerConns[player] = nil
    end
    table.clear(killSoundState.LastHealth)
    table.clear(killSoundState.RecentHits)
end
...
if skyboxState.Enabled and skyboxState.Selected == "FPS Skybox" then
    applyFPSLighting()
else
    restoreSkybox()
end
...
-- ====================== TEXTURA DEL MAPA ======================
local function getAsset(url: string, name: string): string
    local success, result = pcall(function()
        if writefile and isfile and getcustomasset then
            local fileName = "minho_tex_" .. tostring(name or "default") .. ".asset"
...
local TEXTURE_PACKS = {
    ["Minecraft"] = {
        {ids = {7658055825}, url = getAsset(
            "https://raw.githubusercontent.com/jixyuk12nh-maker/Ui/main/texture_pack/Minecraft.png",
            "Minecraft"
        )},
    },
    ["Grods"] = {
        {ids = {7658055825}, url = getAsset(
            "https://raw.githubusercontent.com/jixyuk12nh-maker/World/main/texture_pack/grods.png",
            "Grods"
        )},
    },
}

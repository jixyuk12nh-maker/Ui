...
local function notify(msg, dur)
    ...
    pcall(function() print("[Minho] "..msg) end)
    return false
end
...
function CosmeticsWrap.RankCharm:ResolveMetadata(charmName)
    ...
    local entry = self._overrides[charmName]
    if entry == nil or entry.rankName == nil then return nil end
    ...
    if entry.rankName == nil then return nil end
...
Window = windowOk and windowResult or nil
if Window == nil then
    notify("Failed to create Minho Hub window", 4)
    return
end
...
local function saveOriginalTexture(obj)
    ...
    if texturePackState.OriginalTextures[key] then return end

    local data = {Object = obj}
    if obj:IsA("MeshPart") then
        local ok, value = pcall(function() return obj.TextureID end)
        if ok then data.TextureID = value end
    elseif obj:IsA("Decal") or obj:IsA("Texture") then
        local ok, value = pcall(function() return obj.Texture end)
        if ok then data.Texture = value end
    end
    texturePackState.OriginalTextures[key] = data
end
...
local function restoreOriginalTextures()
    for key, data in pairs(texturePackState.OriginalTextures) do
        if data.Object and data.Object.Parent then
            pcall(function()
                if data.Object:IsA("MeshPart") and data.TextureID ~= nil then
                    data.Object.TextureID = data.TextureID
                elseif (data.Object:IsA("Decal") or data.Object:IsA("Texture")) and data.Texture ~= nil then
                    data.Object.Texture = data.Texture
                end
            end)
        end
...
local function spooferApplyDevice(kind)
    if type(hookmetamethod) ~= "function" or not SPOOFER_DEVICES[kind] then
        return false
    end

    SpooferState.device = kind
    if type(SpooferState.deviceHook) == "function" then
        return true
    end
    ...
    local newIndex = function(self, key)
        ...
        if cur then
            ...
            if self == UserInputService and d[key] ~= nil then
                return d[key]
            end
        end
    ...
    local ok, hooked = pcall(hookmetamethod, game, "__index", newIndex)
    if not ok or type(hooked) ~= "function" then
        SpooferState.device = nil
        SpooferState.deviceHook = nil
        return false
    end

    original = hooked
    SpooferState.deviceHook = original
    return true
end
...
local function spooferResolveTarget(input)
    ...
    if ok and result and result.data and result.data[1] then
        local u=result.data[1]; return {id=u.id,name=u.name,displayName=u.displayName}
    end
    ...
    if ok2 and result2 and result2.data then
        for _,u in ipairs(result2.data) do if u.name and u.name:lower()==input:lower() then return {id=u.id,name=u.name,displayName=u.displayName} end end
        if result2.data[1] then local u=result2.data[1]; return {id=u.id,name=u.name,displayName=u.displayName} end
    end
end
...
local function spooferRestore()
    ...
    for _,clone in pairs(SpooferState.accBackup) do pcall(function() clone:Clone().Parent=char end) end
    SpooferState.partBackup={}; SpooferState.accBackup={}
end
...
local function spooferApplyCharacterParts(userId)
    ...
    if not appearance then
        ...
        if not ok or not result then notify("Avatar load failed: "..userId,3) return end
...
local function spooferApply()
    ...
    task.spawn(function()
        ...
        local data=SpooferState.charCache["user_"..id]
        if not data then
            ...
            SpooferState.charCache["user_"..id]=data
        end
        SpooferState.charApplied=data; spooferApplyCharacterParts(id); notify("Character swapped: "..data.displayName.." ("..id..")",3)
    end)
end
local function spooferRestoreChar()
    ...
    -- Player.Name and Player.DisplayName cannot be restored by client-side assignment.
    -- Original values are retained only for reference.
end

local function spooferIdentity()
    -- Player.Name and Player.DisplayName are read-only client properties.
    -- Keep the controls harmless instead of attempting ineffective writes.
    return false
end
...
CharBox:AddToggle("SpooferCharEnable",{Text="Enable",Default=false,Callback=function(v) SpooferState.charEnabled=v; if v then spooferApply() else spooferRestoreChar(); notify("Spoofer disabled",2) end end})
CharBox:AddInput("SpooferCharTarget",{Text="Target (Username or UserId)",Default="",Placeholder="e.g. builderman or 1",Finished=true,Callback=function(v) if not v or v=="" then return end; task.spawn(function() local data=spooferResolveTarget(v); if data then SpooferState.charUserId=data.id; notify("Found: "..(data.displayName or data.name).." ("..data.id..")",3); if SpooferState.charEnabled then spooferApply() end else notify("User not found: "..v,3) end end) end})

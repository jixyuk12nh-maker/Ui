...
-- Misc / Weapon Selector
-- ============================================================
local WeaponSelectorBox = Misc:AddGroupbox({ Name = "Weapon Selector", Side = 1 })

local function weaponSelectorField(object, key)
    if object == nil then return nil end
...
local function weaponSelectorController()
...
local function weaponSelectorFighter()
    local controller = weaponSelectorController()
    return controller and weaponSelectorField(controller, "LocalFighter") or nil
end

local function weaponSelectorItems()
    local fighter = weaponSelectorFighter()
    if not fighter then return {} end

    local direct = weaponSelectorField(fighter, "Items")
    if type(direct) ~= "table" then return {} end

    return direct
end

local function weaponSelectorEquip(item)
    if not item then return false end
    if weaponSelectorField(item, "IsEquipped") == true then return true end

    local fighter = weaponSelectorField(item, "ClientFighter") or weaponSelectorFighter()
    local data = weaponSelectorField(item, "Data")
    local index = weaponSelectorField(data, "ItemIndex")
...
local function weaponSelectorList()
    local names = {}
    local seen = {}

    for _, item in pairs(weaponSelectorItems()) do
        local name = weaponSelectorField(item, "Name")
...
local WeaponSelectorDropdowns = {}
local WeaponSelectorRefreshQueued = false

local function weaponSelectorEquipByName(name)
    if not name or name == "None" then return end

    for _, item in pairs(weaponSelectorItems()) do
        if weaponSelectorField(item, "Name") == name then
            if weaponSelectorEquip(item) then
                return
            end
        end
    end
...
local function createWeaponSelectorDropdown(index)
    local values = weaponSelectorList()

    WeaponSelectorDropdowns[index] = WeaponSelectorBox:AddDropdown(
        "WeaponSelector" .. index,
        {
            ...
            Callback = function(Value)
                if Value and Value ~= "None" then
                    weaponSelectorEquipByName(Value)
                end
            end,
        }
    )
...
for i = 1, 4 do
    createWeaponSelectorDropdown(i)
end

local function refreshWeaponSelectorDropdowns()
    if WeaponSelectorRefreshQueued then return end
    WeaponSelectorRefreshQueued = true

    task.defer(function()
        WeaponSelectorRefreshQueued = false

        local values = weaponSelectorList()
        for _, dropdown in ipairs(WeaponSelectorDropdowns) do
            if dropdown then
                pcall(function()
...
LocalPlayer.CharacterAdded:Connect(function()
    task.delay(1, refreshWeaponSelectorDropdowns)
end)

task.spawn(function()
    while task.wait(2) do
        refreshWeaponSelectorDropdowns()
    end
end)

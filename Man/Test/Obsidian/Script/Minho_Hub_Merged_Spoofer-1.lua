...
Group:AddButton("Apply To All Weapons", function()
    ...
    if kind == "Skin" then
        if InfoLabel then
            InfoLabel:Set("Skin은 모든 무기에 적용할 수 없습니다.")
            InfoLabel.TextLabel.Visible = true
        end
        return
    end
...
local function setSkyboxSelection(value)
    ...
    if value == "None" then
        restoreSkybox()
    elseif skyboxState.Enabled and value == "FPS Skybox" then
        applyFPSLighting()
    end
end
...
record._playerConn = player.CharacterAdded:Connect(function(char)
...
local record = Records[player]
if record then
    if record._playerConn then
        pcall(function() record._playerConn:Disconnect() end)
        record._playerConn = nil
    end
    if record._conns then
        for _, conn in ipairs(record._conns) do
            pcall(function() conn:Disconnect() end)
        end
        record._conns = {}
    end
...
local SpooferState = getgenv().__MinhoSpooferState or {
    device = nil,
    deviceHook = nil,
    charEnabled = false,
    charUserId = nil,
...
SpooferState.userCache = SpooferState.userCache or {}
getgenv().__MinhoSpooferState = SpooferState

local SpooferCfg = {
    Name={on=false,val="Minho"}, DisplayName={on=false,val="Minho"},
    Winstreak={on=false,val=999}, Level={on=false,val=999},
    CasualWins={on=false,val=9999}, RankedWins={on=false,val=9999},
    CasualWinPercent={on=false,val=100}, RankedWinPercent={on=false,val=100},
    RankedElo={on=false,val=3600}, LeaderboardRank={on=false,val=1},
    FavoriteMap={on=false,val="Arena"}, NametagStatus={on=false,val="Prime"},
    RankTier={on=false,val="Archnemesis"}, Influencer={on=false},
    RobloxEmployee={on=false}, NosniyTeam={on=false},
}

local SPOOFER_MAPS={"Arena","Duel Arena","Rooftop","Dojo","Cave","Beach","Temple","Sky Islands","Volcano","Westown","Station","Docks","Random"}
local SPOOFER_TIERS={"Unranked","Bronze 1","Bronze 2","Bronze 3","Silver 1","Silver 2","Silver 3","Gold 1","Gold 2","Gold 3","Platinum 1","Platinum 2","Platinum 3","Diamond 1","Diamond 2","Diamond 3","Onyx 1","Onyx 2","Onyx 3","Nemesis","Archnemesis"}
local SPOOFER_TAGS={"Prime","Contraband"}
...
local function spooferApplyDevice(kind)
    if not hookmetamethod or not SPOOFER_DEVICES[kind] then return false end
    SpooferState.device=kind
    if SpooferState.deviceHook then return true end
    local old
    old=hookmetamethod(game,"__index",function(self,key)
        local cur=SpooferState.device
        if cur then
            local d=SPOOFER_DEVICES[cur]
            if self==VRService and key=="VREnabled" then return d.VREnabled end
            if self==UserInputService and d[key]~=nil then return d[key] end
        end
        return old(self,key)
    end)
    SpooferState.deviceHook=old
    return true
end

local function spooferResolveTarget(input)
    if not input or input=="" then return nil end
    local numeric=tonumber(input)
    if numeric then
        if SpooferState.userCache[numeric] then return SpooferState.userCache[numeric] end
        ...
        pcall(function()
            ...
            if type(dec)=="table" and dec.id then data.name=dec.name or data.name; data.displayName=dec.displayName or dec.name or data.displayName end
        end)
        SpooferState.userCache[numeric]=data
        return data
    end
    local ok,result=pcall(function()
        local body=HttpService:JSONEncode({usernames={input},excludeBannedUsers=false})
        local response=game:HttpPost("https://users.roblox.com/v1/usernames/users",body,"application/json",true)
        return HttpService:JSONDecode(response)
    end)
    if ok and result and result.data and result.data[1] then
        local u=result.data[1]; local data={id=u.id,name=u.name,displayName=u.displayName}; SpooferState.userCache[data.id]=data; return data
    end
    local ok2,result2=pcall(function()
        local raw=game:HttpGet("https://users.roblox.com/v1/users/search?keyword="..HttpService:UrlEncode(input).."&limit=10",true)
        return HttpService:JSONDecode(raw)
    end)
    if ok2 and result2 and result2.data then
        for _,u in ipairs(result2.data) do if u.name and u.name:lower()==input:lower() then local data={id=u.id,name=u.name,displayName=u.displayName}; SpooferState.userCache[data.id]=data; return data end end
        if result2.data[1] then local u=result2.data[1]; local data={id=u.id,name=u.name,displayName=u.displayName}; SpooferState.userCache[data.id]=data; return data end
    end
end

local SPOOFER_R15={"Head","UpperTorso","LowerTorso","LeftUpperArm","LeftLowerArm","LeftHand","RightUpperArm","RightLowerArm","RightHand","LeftUpperLeg","LeftLowerLeg","LeftFoot","RightUpperLeg","RightLowerLeg","RightFoot"}
local function spooferAttachAccessory(character,humanoid,accessory)
    local handle=accessory:FindFirstChild("Handle")
    if not handle then accessory.Parent=character return end
    handle.CanCollide=false; handle.Massless=true; handle.CanTouch=false; handle.CanQuery=false
    local attachName
    for _,att in ipairs(handle:GetChildren()) do if att:IsA("Attachment") then attachName=att.Name break end end
    accessory.Parent=character
    if attachName then
        local targetAttachment=character:FindFirstChild(attachName,true)
        if targetAttachment then
            local targetPart=targetAttachment.Parent; handle.CFrame=targetAttachment.WorldCFrame
            ...
            weld.C0=targetAttachment.CFrame; weld.C1=handleAtt and handleAtt.CFrame or CFrame.new(); weld.Parent=handle
        end
    end
end

local function spooferBackup(char)
    SpooferState.partBackup={}; SpooferState.accBackup={}
    for _,n in ipairs(SPOOFER_R15) do
        local part=char:FindFirstChild(n)
        if part and part:IsA("BasePart") then
            SpooferState.partBackup[part]={MeshId=part:IsA("MeshPart") and part.MeshId or nil,TextureID=part:IsA("MeshPart") and part.TextureID or nil,Color=part.Color,Material=part.Material,Transparency=part.Transparency,Size=part.Size}
        end
    end
    for _,child in ipairs(char:GetChildren()) do
        if child:IsA("Accessory") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") then SpooferState.accBackup[child]=child:Clone() end
    end
end

local function spooferRestore()
    local char=LocalPlayer.Character; if not char then return end
    for part,data in pairs(SpooferState.partBackup) do
        if part and part.Parent then pcall(function()
            if data.MeshId and part:IsA("MeshPart") then part.MeshId=data.MeshId end
            if data.TextureID and part:IsA("MeshPart") then part.TextureID=data.TextureID end
            part.Color=data.Color; part.Material=data.Material; part.Transparency=data.Transparency
        end) end
    end
    for _,child in ipairs(char:GetChildren()) do
        if child:IsA("Accessory") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") then child:Destroy() end
    end
    for _,clone in pairs(SpooferState.accBackup) do pcall(function() clone.Parent=char end) end
    SpooferState.partBackup={}; SpooferState.accBackup={}
end

local function spooferApplyCharacterParts(userId)
    local char=LocalPlayer.Character; if not char then return end
    local humanoid=char:FindFirstChildOfClass("Humanoid"); if not humanoid then return end
    if not next(SpooferState.partBackup) then spooferBackup(char) end
    local appearance=SpooferState.charCache[userId]
    if not appearance then
        local ok,result=pcall(function() return Players:GetCharacterAppearanceAsync(userId) end)
        if not ok or not result then Library:Notify("Avatar load failed: "..userId,3) return end
        SpooferState.charCache[userId]=result; appearance=result
    end
    for _,n in ipairs(SPOOFER_R15) do
        local src=appearance:FindFirstChild(n); local dst=char:FindFirstChild(n)
        if src and dst then
            if src:IsA("MeshPart") and dst:IsA("MeshPart") then pcall(function() dst.MeshId=src.MeshId; dst.TextureID=src.TextureID end)
            elseif src:IsA("BasePart") and dst:IsA("BasePart") then pcall(function() dst.Color=src.Color; dst.Material=src.Material; dst.Size=src.Size end) end
        end
    end
    for _,child in ipairs(char:GetChildren()) do
    ...
    for _,v in ipairs(appearance:GetChildren()) do
        local clone=v:Clone()
        if clone:IsA("Shirt") or clone:IsA("Pants") or clone:IsA("ShirtGraphic") or clone:IsA("BodyColors") then clone.Parent=char
        elseif clone:IsA("Accessory") then pcall(function() spooferAttachAccessory(char,humanoid,clone) end) end
    end
    local head=char:FindFirstChild("Head"); local face=appearance:FindFirstChild("Head")
    if head and face then for _,dec in ipairs(face:GetChildren()) do if dec:IsA("Decal") then local existing=head:FindFirstChild("face"); if existing then existing.Texture=dec.Texture else local nf=Instance.new("Decal"); nf.Name="face"; nf.Texture=dec.Texture; nf.Parent=head end break end end end
end

local function spooferApply()
    if not SpooferState.charEnabled or not SpooferState.charUserId then return end
    task.spawn(function()
        local id=tonumber(SpooferState.charUserId); if not id then return end
        local data=SpooferState.userCache[id]
        if not data then
            data={id=id,name="User"..id,displayName="User"..id}
            pcall(function() local dec=HttpService:JSONDecode(game:HttpGet("https://users.roblox.com/v1/users/"..id,true)); if dec and dec.id then data={id=dec.id,name=dec.name or ("User"..id),displayName=dec.displayName or dec.name or ("User"..id)} end end)
            SpooferState.userCache[id]=data
        end
        SpooferState.charApplied=data; spooferApplyCharacterParts(id); Library:Notify("Character swapped: "..data.displayName.." ("..id..")",3)
    end)
end
local function spooferRestoreChar()
    if SpooferState.charConn then SpooferState.charConn:Disconnect(); SpooferState.charConn=nil end
    SpooferState.charApplied=nil; spooferRestore()
    pcall(function() LocalPlayer.Name=SpooferState.originalName; LocalPlayer.DisplayName=SpooferState.originalDisplayName end)
end

local function spooferIdentity()
    if SpooferCfg.Name.on then
        pcall(function() LocalPlayer.Name=tostring(SpooferCfg.Name.val) end)
    else
        pcall(function() LocalPlayer.Name=SpooferState.originalName end)
    end
    if SpooferCfg.DisplayName.on then
        pcall(function() LocalPlayer.DisplayName=tostring(SpooferCfg.DisplayName.val) end)
    else
        pcall(function() LocalPlayer.DisplayName=SpooferState.originalDisplayName end)
    end
end

local spooferStats={
    Level="Level",Winstreak="StatisticDuelsWinStreak",CasualWins="StatisticDuelsWon",CasualWinPercent="StatisticDuelsWinPercent",RankedWins="StatisticRankedDuelsPlayed",RankedWinPercent="StatisticRankedDuelsWinPercent",RankedElo="RankedCurrentELO",FavoriteMap="StatisticFavoriteMap"
}
local spooferAttr={Level="Level",Winstreak="StatisticDuelsWinStreak",RankedElo="DisplayELO",NametagStatus="PlayerStatus"}
local spooferBadges={Influencer={"IsInfluencer",true},RobloxEmployee={"IsRobloxEmployee",true},NosniyTeam={"GroupRank",255}}
local spooferOriginal={}
local function spooferRefreshStats()
    local p=LocalPlayer
    for k,a in pairs(spooferAttr) do local n=SpooferCfg[k]; if n and n.on then spooferOriginal[a]=spooferOriginal[a]==nil and p:GetAttribute(a) or spooferOriginal[a]; pcall(function() p:SetAttribute(a,n.val) end) elseif spooferOriginal[a]~=nil then pcall(function() p:SetAttribute(a,spooferOriginal[a]) end); spooferOriginal[a]=nil end end
    for k,spec in pairs(spooferBadges) do local n=SpooferCfg[k]; if n and n.on then spooferOriginal[spec[1]]=spooferOriginal[spec[1]]==nil and p:GetAttribute(spec[1]) or spooferOriginal[spec[1]]; pcall(function() p:SetAttribute(spec[1],spec[2]) end) elseif spooferOriginal[spec[1]]~=nil then pcall(function() p:SetAttribute(spec[1],spooferOriginal[spec[1]]) end); spooferOriginal[spec[1]]=nil end end
end

task.spawn(function() while task.wait(0.5) do pcall(spooferRefreshStats) end end)

local IdentityBox=Spoofer:AddGroupbox({Name="Identity & Stats",Side=1})
IdentityBox:AddToggle("SpooferNameToggle",{Text="Name",Default=false,Callback=function(v) SpooferCfg.Name.on=v; spooferIdentity() end})
IdentityBox:AddInput("SpooferNameInput",{Text="Name",Default="Minho",Finished=true,Callback=function(v) SpooferCfg.Name.val=v; spooferIdentity() end})
IdentityBox:AddToggle("SpooferDNToggle",{Text="Display Name",Default=false,Callback=function(v) SpooferCfg.DisplayName.on=v; spooferIdentity() end})
IdentityBox:AddInput("SpooferDNInput",{Text="Display Name",Default="Minho",Finished=true,Callback=function(v) SpooferCfg.DisplayName.val=v; spooferIdentity() end})
for _,x in ipairs({{"Level","Level",999},{"Winstreak","Winstreak",999},{"CasualWins","Casual Wins",9999},{"RankedWins","Ranked Wins",9999},{"RankedElo","Ranked Elo",3600}}) do
    local key,text,def=x
    IdentityBox:AddToggle("Spoofer"..key.."Toggle",{Text=text,Default=false,Callback=function(v) SpooferCfg[key].on=v; spooferRefreshStats() end})
    IdentityBox:AddInput("Spoofer"..key.."Input",{Text=text,Default=tostring(def),Numeric=true,Finished=true,Callback=function(v) local n=tonumber(v); if n then SpooferCfg[key].val=n; spooferRefreshStats() end end})
end

local ProfileBox=Spoofer:AddGroupbox({Name="Profile",Side=1})
ProfileBox:AddToggle("SpooferFMToggle",{Text="Favorite Map",Default=false,Callback=function(v) SpooferCfg.FavoriteMap.on=v end})
ProfileBox:AddDropdown("SpooferFMDrop",{Text="Map",Values=SPOOFER_MAPS,Default="Arena",Multi=false,Callback=function(v) SpooferCfg.FavoriteMap.val=v end})
ProfileBox:AddToggle("SpooferNTToggle",{Text="Nametag Status",Default=false,Callback=function(v) SpooferCfg.NametagStatus.on=v; spooferRefreshStats() end})
ProfileBox:AddDropdown("SpooferNTDrop",{Text="Status",Values=SPOOFER_TAGS,Default="Prime",Multi=false,Callback=function(v) SpooferCfg.NametagStatus.val=v; spooferRefreshStats() end})

local DeviceBox=Spoofer:AddGroupbox({Name="Device Spoof",Side=1})
DeviceBox:AddDropdown("SpooferDeviceDrop",{Text="Spoof Device",Values={"Off","Computer","Mobile","Console","VR"},Default="Off",Multi=false,Callback=function(v) if v=="Off" then SpooferState.device=nil else spooferApplyDevice(v) end end})

local CharBox=Spoofer:AddGroupbox({Name="Character Spoofer",Side=2})
CharBox:AddToggle("SpooferCharEnable",{Text="Enable",Default=false,Callback=function(v) SpooferState.charEnabled=v; if v then spooferApply() else spooferRestoreChar(); Library:Notify("Spoofer disabled",2) end end})
CharBox:AddInput("SpooferCharTarget",{Text="Target (Username or UserId)",Default="",Placeholder="e.g. builderman or 1",Finished=true,Callback=function(v) if not v or v=="" then return end; task.spawn(function() local data=spooferResolveTarget(v); if data then SpooferState.charUserId=data.id; Library:Notify("Found: "..(data.displayName or data.name).." ("..data.id..")",3); if SpooferState.charEnabled then spooferApply() end else Library:Notify("User not found: "..v,3) end end) end})
local function serverPlayers() local t={}; for _,p in ipairs(Players:GetPlayers()) do if p~=LocalPlayer then t[#t+1]=p.Name.." ("..p.UserId..")" end end; if #t==0 then t[1]="None" end; return t end
SpooferState.playerDrop=CharBox:AddDropdown("SpooferCharTargetDrop",{Text="Pick Server Player",Values=serverPlayers(),Default=nil,Multi=false,Callback=function(v) local id=tonumber(string.match(v or "","%((%d+)%)")); if id then SpooferState.charUserId=id; if SpooferState.charEnabled then spooferApply() end end end})
CharBox:AddButton("SpooferRefreshPlayers",{Text="Refresh Server Player List",Callback=function() if SpooferState.playerDrop and SpooferState.playerDrop.Refresh then pcall(function() SpooferState.playerDrop:Refresh(serverPlayers()) end) end end})
CharBox:AddButton("SpooferApplyNow",{Text="Apply Now",Callback=spooferApply})

local RankBox=Spoofer:AddGroupbox({Name="Rank & Leaderboard",Side=2})
RankBox:AddToggle("SpooferRankToggle",{Text="Rank Tier",Default=false,Callback=function(v) SpooferCfg.RankTier.on=v end})
RankBox:AddDropdown("SpooferRankDrop",{Text="Tier",Values=SPOOFER_TIERS,Default="Archnemesis",Multi=false,Callback=function(v) SpooferCfg.RankTier.val=v end})
RankBox:AddToggle("SpooferLBRankToggle",{Text="Leaderboard Rank",Default=false,Callback=function(v) SpooferCfg.LeaderboardRank.on=v end})
RankBox:AddInput("SpooferLBRankInput",{Text="Leaderboard Rank",Default="1",Numeric=true,Finished=true,Callback=function(v) local n=tonumber(v); if n then SpooferCfg.LeaderboardRank.val=n end end})

local BadgeBox=Spoofer:AddGroupbox({Name="Badges",Side=2})
BadgeBox:AddToggle("SpooferInfToggle",{Text="Influencer",Default=false,Callback=function(v) SpooferCfg.Influencer.on=v; spooferRefreshStats() end})
BadgeBox:AddToggle("SpooferRobToggle",{Text="Roblox Employee",Default=false,Callback=function(v) SpooferCfg.RobloxEmployee.on=v; spooferRefreshStats() end})
BadgeBox:AddToggle("SpooferNosToggle",{Text="Nosniy Team",Default=false,Callback=function(v) SpooferCfg.NosniyTeam.on=v; spooferRefreshStats() end})

LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(1.5)
    if SpooferState.charEnabled and SpooferState.charApplied then
        SpooferState.partBackup={}; SpooferState.accBackup={}; spooferApplyCharacterParts(SpooferState.charApplied.id)
    end
end)
Players.PlayerAdded:Connect(function() task.wait(0.5); if SpooferState.playerDrop and SpooferState.playerDrop.Refresh then pcall(function() SpooferState.playerDrop:Refresh(serverPlayers()) end) end end)
Players.PlayerRemoving:Connect(function() task.wait(0.5); if SpooferState.playerDrop and SpooferState.playerDrop.Refresh then pcall(function() SpooferState.playerDrop:Refresh(serverPlayers()) end) end end)

LocalPlayer.AncestryChanged:Connect(function()
    if not LocalPlayer:IsDescendantOf(game) then
        pcall(spooferRestore)
        pcall(function()
            if SpooferState.charConn then
                SpooferState.charConn:Disconnect()
                SpooferState.charConn = nil
            end
        end)
        SpooferState.charEnabled = false
        SpooferState.device = nil
        pcall(function() LocalPlayer.Name=SpooferState.originalName end)
        pcall(function() LocalPlayer.DisplayName=SpooferState.originalDisplayName end)
    end
end)

--!nonstrict
-- =====================================================
-- Minho Hub — Spoofer (MeshId/TextureID 교체 방식)
-- + attachAccessory 이식
-- + 닉네임 입력으로 서버에 없는 유저도 스푸핑 가능
-- =====================================================
...
playerDrop = nil, -- Dropdown 참조 (갱신용)
}
...
return result.data[1].id, result.data[1].name, result.data[1].displayName
end

-- POST 안 되면 GET 검색으로 우회
local ok2, result2 = pcall(function()
local url = "https://users.roblox.com/v1/users/search?keyword="
.. HttpService:UrlEncode(username) .. "&limit=10"
...
if u.name and u.name:lower() == username:lower() then
return u.id, u.name, u.displayName
end
end
-- 정확 매치 없으면 첫 번째 결과 반환
...
if #list == 0 then
table.insert(list, "없음")
end
return list
end

local function parseUserId(str)
if not str or str == "없음" then return nil end
...
Library:Notify("아바타 로드 실패: " .. userId, 3)
return
end
...
Library:Notify("캐릭터 교체: " .. data.displayName .. " (" .. userId .. ")", 3)
end)
end
...
if char then
local head = char:FindFirstChild("Head")
local backup = State.partBackup[head]
if head and backup and backup.MeshId and head:IsA("MeshPart") then
if head.MeshId ~= backup.MeshId and State.charApplied then
applyCharacterParts(State.charApplied.id)
end
end
...
__newindex = function(, key, val) inner[key] = val end,
...
local headshot = player:FindFirstChild("Headshot")
...
local casual = player:FindFirstChild("Casual")
if casual then
...
local ranked = player:FindFirstChild("Ranked")
if ranked then
...
local CharBox = Spoofer:AddGroupbox({ Name = "Character Spoofer", Side = 1 })
...
restoreCharSpoof()
Library:Notify("스푸퍼 해제", 2)
end
end,
})

-- ⭐ 닉네임 입력 (서버에 없는 유저도 가능)
CharBox:AddInput("CharSpoofName", {
Text = "닉네임 입력 (서버에 없어도 OK)",
Default = "",
Placeholder = "예: builderman",
Finished = true,
Callback = function(v)
if not v or v == "" then return end
Library:Notify("검색 중: " .. v, 2)
task.spawn(function()
local id, name, display = getUserIdFromName(v)
if id then
State.charUserId = id
Library:Notify("찾음: " .. (display or name) .. " (" .. id .. ")", 3)
if State.charEnabled then applyCharSpoof() end
else
Library:Notify("유저를 찾을 수 없음: " .. v, 3)
end
end)
end,
})
...
-- ⭐ UserId 직접 입력
CharBox:AddInput("CharSpoofUserId", {
Text = "User ID 직접 입력",
Default = "",
Placeholder = "예: 1",
Numeric = true,
Finished = true,
Callback = function(v)
State.charUserId = tonumber(v)
if State.charEnabled and State.charUserId then applyCharSpoof() end
end,
})

CharBox:AddButton("RefreshPlayerList", {
Text = "🔄 서버 플레이어 목록 새로고침",
Callback = function()
...
Library:Notify("Dropdown 갱신 미지원 - 라이브러리 확인 필요", 3)
end
end
end,
})
...
local IDBox = Spoofer:AddGroupbox({ Name = "Identity", Side = 1 })

IDBox:AddToggle("NameToggle", {
Text = "Name",
Default = false,
Callback = function(v) Cfg.Name.on = v keepIdentitySafe() end,
})
IDBox:AddInput("NameInput", {
Text = "Name",
...
local BadgeBox = Spoofer:AddGroupbox({ Name = "Badges", Side = 1 })
...
local StatBox = Spoofer:AddGroupbox({ Name = "Stats", Side = 2 })

StatBox:AddToggle("LevelToggle", {
Text = "Level",
Default = false,
Callback = function(v) Cfg.Level.on = v refreshStats() end,
})
StatBox:AddInput("LevelInput", {
Text = "Level",
...
StatBox:AddToggle("CWinsToggle", {
Text = "Casual Wins",
Default = false,
Callback = function(v) Cfg.CasualWins.on = v refreshStats() end,
})
StatBox:AddInput("CWinsInput", {
Text = "Casual Wins",
...
StatBox:AddToggle("RWinsToggle", {
Text = "Ranked Wins",
Default = false,
Callback = function(v) Cfg.RankedWins.on = v refreshStats() end,
})
StatBox:AddInput("RWinsInput", {
Text = "Ranked Wins",
...
local RankBox = Spoofer:AddGroupbox({ Name = "Rank", Side = 2 })

RankBox:AddToggle("RankToggle", {
Text = "Rank Tier",
Default = false,
Callback = function(v) Cfg.RankTier.on = v end,
})
RankBox:AddDropdown("RankDrop", {
Text = "Tier",
Values = TIERS,
Default = "Archnemesis",
Multi = false,
...
local LBBox = Spoofer:AddGroupbox({ Name = "Leaderboard", Side = 2 })

LBBox:AddToggle("LBRankToggle", {
Text = "Leaderboard Rank",
Default = false,
Callback = function(v) Cfg.LeaderboardRank.on = v refreshStats() end,
})
LBBox:AddInput("LBRankInput", {
Text = "Leaderboard Rank",

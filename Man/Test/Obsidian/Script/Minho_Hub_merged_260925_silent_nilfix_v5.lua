...
local function SecretAttachStart()
    ...
    if SecretAttachRagebot.SilentAim
        ...
        hookFn = function(self, ...)
            local prev = SecretAttachRagebot.PrevNamecall
        ...
        local prevFn = hookmetamethod(game, "__namecall", hookFn)

        if type(prevFn) == "function" then
            SecretAttachRagebot.NamecallHook = hookFn
            SecretAttachRagebot.PrevNamecall = prevFn
        else

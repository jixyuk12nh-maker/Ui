...
local originalFinisher = type(original.Finisher) == "table"
and original.Finisher.Name
or original.Finisher

local patchedFinisher = type(patched.Finisher) == "table"
and patched.Finisher.Name
or patched.Finisher

print(("[CosmeticsWrap] Finisher applied to %s: %s -> %s")
:format(
tostring(weaponName),
tostring(originalFinisher),
tostring(patchedFinisher)
))
end
...
local charmName = type(charm) == "table"
and (charm.Name or charm.name)
or charm
...
-- Finisher is included in the weapon data path when the game consumes it
-- from the same ViewModel/weapon record.
local finisher = selection ~= nil and selection.finisher or nil
if type(finisher) == "table" then
finisher = finisher.Name or finisher.name
end

if finisher ~= nil
and not string.match(tostring(finisher), "^NONE_COSMETIC") then
record.Finisher = { Name = finisher }
end
...
local function CosmeticsWrap._loadRankProfile = loadRankProfile
loadRankProfile()
    ...
    -- RankProfile is optional. If it exists, use the game's real ranks.
    ...
    -- Season charm discovery does NOT depend on RankProfile.
    ...
    if type(byVersion) == "table" then
        ...
        CosmeticsWrap.Rank.SEASON_CHARMS = {}
        CosmeticsWrap.Rank.SEASON_CHARM_NAMES = {}
    ...
    -- Only use fallback ranks if the real game data did not provide any.
...
local SeasonCharmNames = CosmeticsWrap.Rank.SEASON_CHARM_NAMES or {}
local SeasonCharmDrop = SeasonCharmOverrideBox:AddDropdown("SeasonCharmOverride", {
Text = "Season Charm", Values = SeasonCharmNames, Multi = false,
})
local SeasonRankNames = CosmeticsWrap.Rank.RANK_NAMES or {}
local SeasonRankDrop = SeasonCharmOverrideBox:AddDropdown("SeasonRankOverride", {
Text = "Rank", Values = SeasonRankNames, Multi = false,
})

local SeasonLeaderboardBox = SeasonCharmOverrideBox:AddInput("SeasonLeaderboardRank", {
Text = "Leaderboard Rank (optional)",
Default = "",
Placeholder = "ex: 1",
})

SeasonCharmOverrideBox:AddButton("Apply", function()
local charmName = SeasonCharmDrop.Value
local rankName = SeasonRankDrop.Value
if charmName == nil or rankName == nil then return end
if CosmeticsWrap.RankCharm == nil
or type(CosmeticsWrap.RankCharm.SetForSeason) ~= "function" then
return
end

local leaderboardRank = nil
local raw = SeasonLeaderboardBox and SeasonLeaderboardBox.Value

if type(raw) == "string" and raw ~= "" then
local parsed = tonumber(string.match(raw, "%d+"))
if parsed ~= nil then
leaderboardRank = math.max(1, math.floor(parsed))
end
elseif type(raw) == "number" then
leaderboardRank = math.max(1, math.floor(raw))
end
...
if selection ~= nil then
local selectedCharm = selection.charm
local selectedCharmName =
type(selectedCharm) == "table"
and (selectedCharm.Name or selectedCharm.name)
or selectedCharm

if selectedCharmName == charmName then
...
task.spawn(function()
    for _ = 1, 6 do
        ...
        if SeasonCharmDrop ~= nil
            ...
            pcall(function()
                SeasonCharmDrop:SetValues(CosmeticsWrap.Rank.SEASON_CHARM_NAMES or {})
            end)
        end

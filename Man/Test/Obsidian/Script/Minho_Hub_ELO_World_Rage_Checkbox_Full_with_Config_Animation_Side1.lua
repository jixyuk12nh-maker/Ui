...
local UERage_Toggle = Rage:AddCheckbox("UEAssistedRage", {
...
local Underground_Toggle = Rage:AddCheckbox("Underground", {
    Text = "Underground",
    Default = false,
...
local TexturePackBox = World:AddGroupbox({ Name = "Texture Pack", Side = 1 })
...
local TEXTURE_PACKS = {
    ["Minecraft"] = {
        {ids = {7658055825}, url = "https://raw.githubusercontent.com/jixyuk12nh-maker/Ui/main/texture_pack/Minecraft.png"},
    },
    ["Grods"] = {
        {ids = {7658055825}, url = "https://raw.githubusercontent.com/jixyuk12nh-maker/Ui/main/texture_pack/Grods.png"},
    },
}
...
texturePackState.Enabled = texturePackState.Enabled == true
getgenv().__MinhoTexturePackState = texturePackState

local function saveOriginalTexture(decalOrTexture)
    if not decalOrTexture then return end
    local key = decalOrTexture:GetDebugId()
    if not texturePackState.OriginalTextures[key] then
        texturePackState.OriginalTextures[key] = {
            Object = decalOrTexture,
            Texture = decalOrTexture.Texture,
        }
    end
end
...
local function applyTexturePack(packName)
    ...
    -- 맵의 모든 Decal/Texture 찾아서 교체
    local function processObject(obj)
        if obj:IsA("Decal") or obj:IsA("Texture") then
            local textureId = obj.Texture
            -- 숫자 ID 추출
            local idMatch = textureId:match("(%d+)")
            if idMatch then
                local idNum = tonumber(idMatch)
                for _, texData in ipairs(pack) do
                    for _, targetId in ipairs(texData.ids) do
                        if idNum == targetId then
                            saveOriginalTexture(obj)
                            pcall(function() obj.Texture = getAsset(texData.url) end)
                            break
                        end
                    end
                end
...
TexturePackBox:AddDropdown("TexturePack", {
    ...
    Default = texturePackState.Selected,
    Multi = false,
    Callback = function(Value)
...
local KillSoundBox = Visuals:AddGroupbox({ Name = "Kill Sound", Side = 1 })
...
local Noclip_Toggle = FlyNoclipBox:AddCheckbox("Noclip", {
...
local Fly_Toggle = FlyNoclipBox:AddCheckbox("FlyEnabled", {
...
FlyNoclipBox:AddKeyPicker("ThirdPersonKey", {
    Text = "Third Person",
    Default = "None",
    Mode = "Toggle",
    SyncToggleState = true,
...
local AnimationBox = Character:AddGroupbox({ Name = "Animation Player", Side = 1 })
...
local MiscBox = Misc:AddGroupbox({ Name = "Misc", Side = 1 })

MiscBox:AddCheckbox("ELOLookup", {
    Text = "ELO Lookup",
    Default = false,
    Callback = function(Value)
        getgenv().__MinhoELOLookupEnabled = Value
    end
})

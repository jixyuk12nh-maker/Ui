...
-- ============================================================
-- Secret: Attach Ragebot (fixed weapon type + target mode)
-- ============================================================
local SecretAttachRagebot = {
    Enabled = false,
    AttachTime = 0.10,
    HeightOffset = 0,
    WeaponType = "Primary",     -- Primary | Secondary | Melee | Utility
    TargetMode = "Closest",     -- Closest | Crosshair | Random
    BaseRadius = 100,
    RadiusFactor = 0.5,
    Connection = nil,
    AttachedUntil = 0,
...
local SecretAttachLocalRootCache = nil

local function SecretAttachLocalFighter()
    return CosmeticsWrap.localFighter()
end

local function SecretAttachItemField(item, key)
    if item == nil then return nil end
    local ok, value = pcall(function() return item[key] end)
    if ok and value ~= nil then return value end
    local okGet, getValue = pcall(function() return item:Get(key) end)
    if okGet and getValue ~= nil then return getValue end
    local data = item.Data
    if data ~= nil then
        local okData, dataValue = pcall(function() return data[key] end)
        if okData and dataValue ~= nil then return dataValue end
    end
    local okAttr, attr = pcall(function() return item:GetAttribute(key) end)
    if okAttr and attr ~= nil then return attr end
    return nil
end

local function SecretAttachItemClass(item)
    if item == nil then return nil end

    local info = SecretAttachItemField(item, "Info")
    local class = info and SecretAttachItemField(info, "Class")
    if class ~= nil then
        return class
    end

    local itemType = SecretAttachItemField(item, "Type")
    if itemType == "Melee" or itemType == "Utility" then
        return itemType
    end

    local idx = SecretAttachItemField(item, "ItemIndex")
    if idx == nil then
        local data = SecretAttachItemField(item, "Data")
        idx = data and SecretAttachItemField(data, "ItemIndex") or nil
    end

    idx = tonumber(idx)
    if idx ~= nil then
        return ({ "Primary", "Secondary", "Melee" })[idx]
    end
    ...
    if fighter ~= nil then
        ...
        if type(items) == "table" then
            for _, entry in pairs(items) do
                if type(entry) == "table" and rawget(entry, "IsEquipped") == true then
...
local function SecretAttachFindItemByClass(class)
    ...
    for _, entry in pairs(items) do
        if type(entry) == "table" and SecretAttachItemClass(entry) == class then
            local score = 0
            if rawget(entry, "IsEquipped") == true then score = score + 100 end
            if class == "Primary" or class == "Secondary" then
                local ammo = SecretAttachItemField(entry, "Ammo")
                if ammo == nil or tonumber(ammo) == nil or tonumber(ammo) > 0 then
                    score = score + 10
                end
            end
...
local function SecretAttachEquipItem(item)
    ...
    if fighter ~= nil and idx ~= nil then
        local ok = pcall(function() fighter:EquipItem(idx) end)
        if ok then return true end
    end

    local ok = pcall(function() item:Equip() end)
    return ok
end
...
local function SecretAttachHasAmmo(item)
    ...
    local ammo = SecretAttachItemField(item, "Ammo")
    if ammo == nil then
        return false
    end

    local numericAmmo = tonumber(ammo)
    if numericAmmo == nil then
        return false
    end

    return numericAmmo > 0
end
...
local function SecretAttachIsReloading(item)
    ...
    return SecretAttachItemField(item, "IsReloading") == true
end
...
local function SecretAttachGet(item, key)
    if item == nil then return nil end
    ...
    local data = SecretAttachItemField(item, "Data")
    local hit = SecretAttachItemField(data, key)
    if hit ~= nil then return hit end

    return SecretAttachItemField(item, key)
end
...
local function SecretAttachEnum(name)
    ...
    if ok and lib ~= nil then
        local success, value = pcall(function()
            return lib:ToEnum(name)
        end)
        if success and value ~= nil then
            return value
        end

        local map = rawget(lib, "_to_enum")
        if type(map) == "table" and map[name] ~= nil then
            return map[name]
        end
    end
...
local function SecretAttachEncodeCFrame(cf)
    ...
    if ok and util ~= nil then
        ...
        if success and type(encoded) == "table" then
            return encoded
        end
    end
...
local function SecretAttachObjectId(item)
    return SecretAttachGet(item, "ObjectID")
end

local function SecretAttachHitDescriptor(originCF, part)
    if part == nil then return nil, nil end

    local ok, point = pcall(function()
        return part:GetClosestPointOnSurface(originCF.Position)
    end)
    ...
    return part, SecretAttachEncodeCFrame(CFrame.new(point))
end

local function SecretAttachShootAt(fromCF, toCF, part)
    local remote = SecretAttachUseItemRemote()
    local item = SecretAttachGetEquippedItem()
    ...
    local hitPart, hitCF = SecretAttachHitDescriptor(fromCF, part)
    if hitPart == nil or hitCF == nil then
        return false
    end
    ...
    local info = SecretAttachItemField(item, "Info")
    local isRaycast = SecretAttachItemField(info, "IsRaycast") == true
    local outer = {
        [utf8.char(1)] = body,
    }

    if isRaycast then
        outer[utf8.char(2)] = true
    end

    local ok = pcall(function()
        remote:FireServer(
            objectId,
            SecretAttachEnum("StartShooting"),
            outer,
            nil
        )
    end)
...
local function SecretAttachTryShoot(target)
    ...
    local myRoot = SecretAttachLocalRootCache or SecretAttachGetRoot(LocalPlayer)
    if not myRoot then return false end

    local origin = myRoot.Position
    local look = CFrame.lookAt(origin, target.Head.Position)
    local fromCF = CFrame.new(origin) * CFrame.Angles(look:ToOrientation())
    local toCF = CFrame.new(target.Head.Position) * CFrame.Angles(look:ToOrientation())

    return SecretAttachShootAt(fromCF, toCF, target.Head)
end
...
local function SecretAttachTryMelee(target)
    ...
    if not item or not myRoot or not remote or not objectId then
        return false
    end

    local targetPart = target.Head
    local far = SecretAttachGlueAcquire(myRoot, targetPart, true)
    if far == nil then
        return false
    end

    local fromCF = CFrame.new(far.Position)
    local toCF = CFrame.new(far.Position)
    local hitCF = SecretAttachEncodeCFrame(CFrame.new(targetPart.Position))
...
local function SecretAttachEvade()
    if SecretAttachRagebot.Enabled then
        SecretRandomEvasionStop()
        SecretRandomEvasionStart()
    end
end
...
local function SecretAttachReset()
    ...
    SecretAttachDesyncUnbind()

    local root = SecretAttachLocalRootCache or SecretAttachGetRoot(LocalPlayer)
    if root then
...
local function SecretAttachStart()
    ...
    SecretAttachRagebot.Connection = RunService.Heartbeat:Connect(function()
        ...
        SecretAttachLocalRootCache = SecretAttachGetRoot(LocalPlayer)
        local myRoot = SecretAttachLocalRootCache
        if not myRoot then
            SecretAttachReset()
            return
        end

        -- Fixed weapon: force the selected class every frame.
        ...
        if rawget(wantedItem, "IsEquipped") ~= true then
            SecretAttachStopEvade()
            SecretAttachEquipItem(wantedItem)
            return
        end

        -- Utility is intentionally held without an attack.
        if SecretAttachRagebot.WeaponType == "Utility" then
            SecretAttachEvade()
            return
        end

        if not SecretAttachHasAmmo(wantedItem) or SecretAttachIsReloading(wantedItem) then
            SecretAttachEvade()
            return
        end
        ...
        if not target or not target.Head or not target.Root or not SecretAttachIsAlive(target.Player) then
            SecretAttachEvade()
            SecretAttachRagebot.AttachedUntil = 0
            return
        end
        ...
        if SecretAttachRagebot.AttachedUntil <= os.clock() then
            SecretAttachRagebot.AttachedUntil = os.clock()
                + math.max(0.01, math.min(5, SecretAttachRagebot.AttachTime))
        end
        ...
        -- Apply the temporary replication position only for the current tick.
        SecretAttachDesyncPush(myRoot)

        if os.clock() <= SecretAttachRagebot.AttachedUntil then
            if SecretAttachRagebot.WeaponType == "Melee" then
...
getgenv().AttachRagebot = {
    ...
    SetBaseRadius = function(v)
        if type(v) == "number" then SecretAttachRagebot.BaseRadius = math.clamp(v, 5, 1000000) end
    end,
    SetRadiusFactor = function(v)
        if type(v) == "number" then SecretAttachRagebot.RadiusFactor = math.clamp(v, 0, 1) end
    end,
...
SecretBox:AddSlider("SecretAttachBaseRadius", {
    Text = "Evade Radius",
    Default = 100,
    Min = 5,
    Max = 1000000,
    Rounding = 0,
    Callback = function(Value)
        SecretAttachRagebot.BaseRadius = math.clamp(Value, 5, 1000000)
        if getgenv().RandomEvasion and getgenv().RandomEvasion.SetBaseRadius then
            getgenv().RandomEvasion.SetBaseRadius(Value)
        end
    end,
})

SecretBox:AddSlider("SecretAttachRadiusFactor", {
    Text = "Evade Radius Factor",
    Default = 0.5,
    Min = 0,
    Max = 1,
    Rounding = 2,
    Callback = function(Value)
        SecretAttachRagebot.RadiusFactor = math.clamp(Value, 0, 1)
        if getgenv().RandomEvasion and getgenv().RandomEvasion.SetRadiusFactor then
            getgenv().RandomEvasion.SetRadiusFactor(Value)
        end
    end,
})

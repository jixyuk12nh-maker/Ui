...
local function SecretAttachReadField(item, key)
    ...
    if data then
        local v = data:GetAttribute(key)
        if v ~= nil then return v end
        local obj = data:FindFirstChild(key)
        if obj and obj.Value ~= nil then return obj.Value end
    end
...
local function SecretAttachTryShoot(target)
    if not SecretAttachRagebot.AutoShoot or not target then return false end
    ...
    -- Prefer the supplied Rivals adapter when available.
    if SecretAttachCall("shootAt", aim, aim, head) then
        return true
    end

    -- Fallback: fire a common Tool RemoteEvent if the test environment exposes one.
    if item then
        for _, obj in ipairs(item:GetDescendants()) do
            if obj:IsA("RemoteEvent") and (obj.Name == "Shoot" or obj.Name == "Fire" or obj.Name == "Attack") then
                pcall(function()
                    obj:FireServer(head.Position, head)
                end)
                return true
            end
        end
...
local function SecretAttachTryMelee(target)
    if not SecretAttachRagebot.AutoMelee or not target then return false end
...
local function SecretAttachStart()
    ...
    SecretAttachRagebot.Connection = RunService.Heartbeat:Connect(function()
        ...
        local empty = not SecretAttachHasAmmo(item)
        local reloading = SecretAttachIsReloading(item)

        -- No ammo / reloading: leave the target and use Random Evasion.
        if empty or reloading then
            SecretAttachEvade()
            return
        end
        ...
        -- Attach position: target head + configurable vertical offset.
        local attachPos = target.Head.Position
            + Vector3.new(0, SecretAttachRagebot.HeightOffset, 0)

        myRoot.CFrame = CFrame.lookAt(attachPos, target.Head.Position)

        -- Attack after attaching, while the configured attach window is active.
        if os.clock() <= SecretAttachRagebot.AttachedUntil then
            local itemName = tostring(SecretAttachReadField(item, "Name") or item.Name or "")
            local isMelee = itemName == "Knife"
                or itemName:lower():find("knife", 1, true) ~= nil
                or itemName:lower():find("melee", 1, true) ~= nil
...
getgenv().AttachRagebot = {
    ...
    SetAutoShoot = function(v)
        SecretAttachRagebot.AutoShoot = v == true
    end,
    SetAutoMelee = function(v)
        SecretAttachRagebot.AutoMelee = v == true
    end,
...
SecretBox:AddCheckbox("SecretAttachAutoShoot", {
    Text = "Auto Shoot",
    Default = true,
    Callback = function(Value)
        SecretAttachRagebot.AutoShoot = Value
    end,
})

SecretBox:AddCheckbox("SecretAttachAutoMelee", {
    Text = "Auto Melee",
    Default = true,
    Callback = function(Value)
        SecretAttachRagebot.AutoMelee = Value
    end,
})

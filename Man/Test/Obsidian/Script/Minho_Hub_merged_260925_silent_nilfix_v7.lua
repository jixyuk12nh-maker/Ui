...
local SecretAttachLastMelee = 0

local function SecretAttachTryMelee(target)
    if not target or not target.Head then return false end

    local now = os.clock()
    if now - SecretAttachLastMelee < 0.5 then
        return false
    end
    SecretAttachLastMelee = now
...
local function SecretAttachStart()
    ...
    if SecretAttachRagebot.SilentAim
        and not SecretAttachRagebot.NamecallHook
        ...
        hookFn = function(self, ...)
            ...
            if not SecretAttachRagebot.Enabled
                or not SecretAttachRagebot.SilentAim
                or (type(checkcaller) == "function" and checkcaller()) then
                return prev(self, ...)
            end

            local method
            if type(getnamecallmethod) == "function" then
                local ok, result = pcall(getnamecallmethod)
                if ok then
                    method = result
                end
            end
            ...
            if method == "Raycast" and self == workspace then
                ...
                if myRoot and typeof(origin) == "Vector3"
                    and typeof(direction) == "Vector3"
                    and (origin - myRoot.Position).Magnitude < 16 then
                    local target = SecretAttachRagebot.Target
                    if target and target.Head and target.Head.Parent then
                        local delta = target.Head.Position - origin
                        if delta.Magnitude > 0 and direction.Magnitude > 0 then
                            local newDir = delta.Unit * direction.Magnitude
                            return prev(self, origin, newDir, params)
                        end
                    end
                end
            end
        ...
        local installedPrev = hookmetamethod(game, "__namecall", hookFn)

        if type(installedPrev) == "function" then
            prevFn = installedPrev
            SecretAttachRagebot.NamecallHook = hookFn
            SecretAttachRagebot.PrevNamecall = installedPrev
        else
            SecretAttachRagebot.NamecallHook = nil
            SecretAttachRagebot.PrevNamecall = nil
            if type(restorefunction) == "function" then
                pcall(restorefunction, hookFn)
            end
        end
    end
...
local function SecretAttachStop()
    ...
    -- Keep the namecall hook installed and only disable its behavior.
    -- This avoids breaking the hook chain when an executor cannot safely
    -- restore a hookmetamethod hook with restorefunction.
...
SecretBox:AddCheckbox("SecretAttachSilentAim", {
    Text = "Silent Aim (Raycast)",
    Default = true,
    Callback = function(Value)
        SecretAttachRagebot.SilentAim = Value
    end,
})

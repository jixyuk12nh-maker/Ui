...
local function weaponSelectorGetList()
    local weapons = {}
...
local function weaponSelectorEquipOwned(weaponName)
    ...
    if not items then return false, "파이터 없음" end
...
local function weaponSelectorSwitchSlot(weaponName)
    ...
    if not switchRemote then return false, "SwitchItems Remote 없음" end
    ...
    if ok then
        return true, "슬롯 " .. tostring(slot) .. "에 요청됨"
    end

    return false, tostring(err)
end

local function weaponSelectorPickAhead(weaponName)
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    local duels = remotes and remotes:FindFirstChild("Duels")
    local pickRemote = duels and duels:FindFirstChild("PickWeaponsAheadOfTime")
    if not pickRemote then return false, "PickWeapons Remote 없음" end

    local ok, err = pcall(function()
        pickRemote:FireServer(weaponName)
    end)
    ...
    WeaponSelectorDropdowns[index] = WeaponSelectorBox:AddDropdown(
        "WeaponSelector" .. index,
        {
            ...
            Callback = function(value)
                WeaponSelectorSelections[index] = value

                if value and value ~= "None" then
                    weaponSelectorApply(value)
                end
            end,
        }
    )
...
local function refreshWeaponSelectorDropdowns()
    ...
    task.defer(function()
        ...
        for _, dropdown in pairs(WeaponSelectorDropdowns) do
            if dropdown then
                pcall(function()

...
local seasonCharmFilter = nil

local function filterCharmList(list)
    if seasonCharmFilter == nil then return list end
    local out = { "None" }
    for _, name in ipairs(list) do
        if name == seasonCharmFilter then
...
Callback = function(value)
    if value == "All" or value == nil then
        seasonCharmFilter = nil
    else
        seasonCharmFilter = value
    end

    if KindDropdown and KindDropdown.Value == "Charm" then
        local class = ClassDropdown.Value
        local weapon = WeaponDropdown.Value
        local list = cosmeticsOf(class, weapon, "Charm")
        list = filterCharmList(list)
        if CosmeticDropdown then
            CosmeticDropdown:SetValues(list)
            if CosmeticDropdown.Value ~= seasonCharmFilter then
...
local SeasonLeaderboardRank = SeasonCharmOverrideBox:AddInput("SeasonLeaderboardRank", {
Text = "Leaderboard Rank (비우면 1)",
Default = "",
Placeholder = "ex: 1",
})
...
Group:AddButton("Apply to All Weapons", function()
    ...
    if kind == "Skin" then return end

    local allWeapons = {}
    local items = getRivalsItems()
    if items then
        for weaponName, data in pairs(items) do
            if type(data) == "table" and data.Class then
    ...
    for class, weapons in pairs(byClass) do
    ...
    local slot = string.lower(kind)
    local cleared = (not name or name == NONE_LABEL)

    for weapon in pairs(allWeapons) do
        if cleared then
            local selection = CosmeticsWrap.selections[weapon]
            if selection then
                selection[slot] = nil
                if next(selection) == nil then
                    CosmeticsWrap.clear(weapon)

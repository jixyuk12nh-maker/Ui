...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        -- Mobile uses a checkbox; desktop uses key-only display.
        local KeybindsToggle = { Normal = not (Library.IsMobile and KeyPicker.Mode == "Toggle") }
        do
            ...
            function KeybindsToggle:SetNormal(Normal)
                ...
                local TouchDevice = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
                local MobileDevice = Library.IsMobile or TouchDevice
                Checkbox.Visible = MobileDevice and not Normal
            end
        ...
        function KeyPicker:Update()
            ...
            if KeybindsToggle.Loaded then
                -- Mobile: always show a checkbox + feature name.
                -- Desktop: show [Key] + feature name.
                local TouchDevice = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
                local MobileDevice = Library.IsMobile or TouchDevice

                KeybindsToggle:SetNormal(not MobileDevice)

                if MobileDevice then
                    KeybindsToggle:SetText(KeyPicker.Text)
                else
                    KeybindsToggle:SetText(("[%s] %s"):format(KeyPicker.DisplayValue, KeyPicker.Text))
                end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        -- Keep the keybind panel height automatic, but give it a wider fixed width.
        Library.KeybindFrame.AutomaticSize = Enum.AutomaticSize.Y
        Library.KeybindFrame.Size = UDim2.fromOffset(220, 0)

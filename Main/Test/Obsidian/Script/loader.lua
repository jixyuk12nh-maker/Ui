...
if antiAimState.MTInstalled and antiAimState.MT and antiAimState.OldNamecall then
    pcall(function() antiAimState.MT.__namecall = antiAimState.OldNamecall end)
...
local function installAntiAimHook()
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "FireServer" and antiAimState.Enabled then
            if self.Name == "UpdateCameraRotation" then
                local cfg = _G.AntiAimPoseConfig
                if cfg and cfg.enabled then
                    local cur = camCtrl.Rotation or Vector2.zero
                    local pitch, yaw = cur.X or 0, cur.Y or 0
                    ...
                    local enc = util:EncodeCameraRotation(rot)
                    return oldNamecall(self, enc, nil)
                end
            end
        end
...
AntiAimBox:AddCheckbox("AntiAimEnabled", {
    ...
    Callback = function(Value)
        ...
        _G.AntiAimPoseConfig = {
            enabled = Value, pitch = antiAimState.Pitch,
            yaw = antiAimState.Yaw, underground = antiAimState.Underground,
        }
        if Value then installAntiAimHook() else
            _G.AntiAimPoseConfig = nil
